  ...
  var Event = YAHOO.util.Event,
    ...
    getMatch = function (el, selector, container) {
      ...
      if (!el || el === container) {
        ...
        returnVal = YAHOO.util.Selector.test(el, selector)
          ? el
          : getMatch(el.parentNode, selector, container);
      }
  ...
  Lang.augmentObject(Event, {
    ...
    delegate: function (container, type, fn, filter, obj, overrideContext) {
      ...
      if (Lang.isString(filter) && !YAHOO.util.Selector) {
        YAHOO.log(
          'Using a CSS selector to define the filtering criteria for a delegated listener requires the Selector Utility.',
          'error',
          'Event'
        );
        return false;
      ...
      if (type == 'mouseenter' || type == 'mouseleave') {
        if (!Event._createMouseDelegate) {
          YAHOO.log(
            'Delegating a ' + type + ' event requires the event-mouseenter module.',
            'error',
            'Event'
          );
          return false;
        ...
        fnDelegate = Event._createDelegate(
          function (event, matchedEl, container) {
      ...
      if (nodes) {
        ...
        if (nodes.length) {
          ...
          for (i = 0; (node = nodes[i++]); ) {
            if (
              node.id === id ||
              (node.attributes && node.attributes.id && node.attributes.id.value === id)
            ) {
              ret.push(node);
            }
          }
  ...
  Selector = {
    ...
    query: function (selector, root, firstOnly, skipNative) {
      ...
      if (selector && fn) {
        ...
        if (
          !skipNative && // already done if skipping
          (!useNative || root.tagName)
        ) {
          // split native when element scoping is needed
        ...
        for (i = 0; (query = queries[i++]); ) {
          ...
          if (!firstOnly) {
            // coerce DOM Collection to Array
        ...
        if (queries.length > 1) {
          // remove dupes and sort by doc order
    ...
    _nativeQuery: function (selector, root, one) {
      ...
      try {
        ...
        // fallback to brute if available
    ...
    filter: function (nodes, selector) {
      ...
      if (nodes && selector) {
        ...
        Y.log(
          'invalid filter input (nodes: ' + nodes + ', selector: ' + selector + ')',
          'warn',
          'Selector'
        );
      }
      ...
      if (node && node.tagName) {
        ...
        if (!root && !Y_DOM_inDoc(node)) {
          ...
          if (parent) {
            ...
            // only use frag when no parent to query
  ...
  SelectorCSS2 = {
    ...
    _children: function (node, tag) {
      ...
      if (node.children && tag && node.children.tags) {
        ...
        // only HTMLElements have children
    ...
    _bruteQuery: function (selector, root, firstOnly) {
      ...
      if (token) {
        ...
        if (root.getElementsByTagName) {
          // non-IE lacks DOM api on doc frags
        ...
        testLoop: while (tmpNode && tmpNode.tagName) {
          ...
          if (j && !pass) {
            while ((test = tests[--j])) {
              ...
              if (
                (operator === '=' && value !== test[2]) || // fast path for equality
                (typeof operator !== 'string' && // protect against String.test monkey-patch (Moo)
                  operator.test &&
                  !operator.test(value)) || // regex test
                (!operator.test && // protect against RegExp as function (webkit)
                  typeof operator === 'function' &&
                  !operator(tmpNode, test[0], test[2]))
              ) {
                // function test
                ...
                if ((tmpNode = tmpNode[path])) {
                  while (
                    tmpNode &&
                    (!tmpNode.tagName || (token.tagName && token.tagName !== tmpNode.tagName))
                  ) {
    ...
    _parsers: [
      ...
      },
      {
        ...
        fn: function (match, token) {
          ...
          if (test) {
            // reorder match array and unescape special chars for tests
            if (match[2]) {
              match[2] = match[2].replace(/\\/g, '');
            }
            return [match[2], test];
          } else {
            // selector token not supported (possibly missing CSS3 module)
            return false;
          }
        },
      },
    ...
    _tokenize: function (selector) {
      ...
      if (!found || selector.length) {
        // not fully parsed
        Y.log(
          'query: ' + query + ' contains unsupported token in: ' + selector,
          'warn',
          'Selector'
        );
        tokens = [];
        ...
        if (YAHOO.lang.isArray(currentNode.children)) {
          ...
          for (var k = 0; k < currentChildren.length; k++) {
            ...
            if (oColumn.type && !oColumn.formatter) {
              YAHOO.log(
                'The property type has been' + ' deprecated in favor of formatter',
                'warn',
                oColumn.toString()
              );
            ...
            if (oColumn.text && !YAHOO.lang.isValue(oColumn.label)) {
              YAHOO.log(
                'The property text has been' + ' deprecated in favor of label',
                'warn',
                oColumn.toString()
              );
            ...
            if (oColumn.parser) {
              YAHOO.log('The property parser is no longer supported', 'warn', this.toString());
            }
            if (
              oColumn.sortOptions &&
              (oColumn.sortOptions.ascFunction || oColumn.sortOptions.descFunction)
            ) {
              YAHOO.log(
                'The properties sortOptions.ascFunction and ' +
                  ' sortOptions.descFunction have been deprecated in favor ' +
                  ' of sortOptions.sortFunction',
                'warn',
                oColumn.toString()
              );
            }
...
YAHOO.lang.augmentObject(YAHOO.widget.Column, {
  ...
  formatCheckbox: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatCheckbox() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatCheckbox()',
      'warn',
  ...
  formatCurrency: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatCurrency() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatCurrency()',
      'warn',
  ...
  formatDate: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatDate() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatDate()',
      'warn',
  ...
  formatEmail: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatEmail() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatEmail()',
      'warn',
  ...
  formatLink: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatLink() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatLink()',
      'warn',
  ...
  formatNumber: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatNumber() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatNumber()',
      'warn',
  ...
  formatSelect: function (elCell, oRecord, oColumn, oData) {
    YAHOO.log(
      'The method YAHOO.widget.Column.formatSelect() has been' +
        ' deprecated in favor of YAHOO.widget.DataTable.formatDropdown()',
      'warn',
...
YAHOO.widget.Column.prototype = {
  ...
  getColSpan: function () {
    YAHOO.log(
      'The method getColSpan() has been' + ' deprecated in favor of getColspan()',
      'warn',
      this.toString()
    );
  ...
  getColEl: function () {
    YAHOO.log(
      'The method getColEl() has been' + ' deprecated in favor of getThEl()',
      'warn',
      this.toString()
    );
    return this.getThEl();
  ...
  getIndex: function () {
    YAHOO.log(
      'The method getIndex() has been' + ' deprecated in favor of getKeyIndex()',
      'warn',
      this.toString()
    );
  ...
  format: function () {
    YAHOO.log(
      'The method format() has been deprecated in favor of the ' + 'DataTable method formatCell()',
      'error',
      this.toString()
    );
  },
  ...
  YAHOO.extend(YAHOO.widget.ColumnDD, YAHOO.util.DDProxy, {
    ...
    _resizeProxy: function () {
      ...
      YAHOO.util.Dom.setStyle(
        this.pointer,
        'height',
        this.table.parentNode.offsetHeight + 10 + 'px'
      );
      ...
      YAHOO.util.Dom.setStyle(
        dragEl,
        'height',
        this.datatable.getContainerEl().offsetHeight + 'px'
      );
      YAHOO.util.Dom.setStyle(
        dragEl,
        'width',
        parseInt(YAHOO.util.Dom.getStyle(dragEl, 'width'), 10) + 4 + 'px'
      );
...
if (YAHOO.util.DD) {
  YAHOO.extend(YAHOO.util.ColumnResizer, YAHOO.util.DDProxy, {
    ...
    onMouseDown: function (e) {
      ...
      this.nLinerPadding =
        (parseInt(YAHOO.util.Dom.getStyle(this.headCellLiner, 'paddingLeft'), 10) | 0) +
  ...
  RS.prototype = {
    ...
    addRecord: function (oData, index) {
      if (lang.isObject(oData)) {
        ...
        YAHOO.log(
          'Added Record at index ' + index + ' with data ' + lang.dump(oData),
          'info',
          this.toString()
        );
        return oRecord;
      } else {
        YAHOO.log('Could not add Record with data' + lang.dump(oData), 'info', this.toString());
        return null;
      }
    },
    ...
    addRecords: function (aData, index) {
      if (lang.isArray(aData)) {
        ...
        YAHOO.log(
          'Added 1 Record at index ' + index + ' with data ' + lang.dump(aData),
          'info',
          this.toString()
        );
        return oRecord;
      } else {
        YAHOO.log('Could not add Records with data ' + lang.dump(aData), 'info', this.toString());
        return null;
      }
    },
    ...
    setRecord: function (oData, index) {
      if (lang.isObject(oData)) {
        ...
        YAHOO.log(
          'Set Record at index ' + index + ' with data ' + lang.dump(oData),
          'info',
          this.toString()
        );
        return oRecord;
      } else {
        YAHOO.log('Could not set Record with data' + lang.dump(oData), 'info', this.toString());
        return null;
      }
    },
    ...
    setRecords: function (aData, index) {
      ...
      YAHOO.log('Set ' + j + ' Record(s) at index ' + index, 'info', this.toString());

      if (a.length && !added.length) {
        YAHOO.log('Could not set Records with data ' + lang.dump(aData), 'info', this.toString());
      }
    ...
    updateRecordValue: function (record, sKey, oData) {
      ...
      if (oRecord) {
        ...
        YAHOO.log(
          'Could not update key ' + sKey + ' for Record ' + record,
          'error',
          this.toString()
        );
      }
    ...
    deleteRecord: function (index) {
      if (lang.isNumber(index) && index > -1 && index < this.getLength()) {
        ...
        YAHOO.log(
          'Record deleted at index ' + index + ' and containing data ' + lang.dump(oData),
          'info',
          this.toString()
        );
        return oData;
    ...
    deleteRecords: function (index, range) {
      ...
      if (lang.isNumber(index) && index > -1 && index < this.getLength()) {
        ...
        this.fireEvent('recordsDeleteEvent', {
          data: deletedData,
          deletedData: deletedObjects,
          index: index,
        });
        YAHOO.log(
          range +
            'Record(s) deleted at index ' +
            index +
            ' and containing data ' +
            lang.dump(deletedObjects),
          'info',
          this.toString()
        );
...
(function () {
  ...
  YAHOO.widget.DataTable = function (elContainer, aColumnDefs, oDataSource, oConfigs) {
    ...
    if (!this._oDataSource) {
      YAHOO.log(
        'Could not instantiate DataTable due to an invalid DataSource',
        'error',
        this.toString()
      );
      return;
    ...
    if (!this._oColumnSet) {
      YAHOO.log(
        'Could not instantiate DataTable due to an invalid ColumnSet',
        'error',
        this.toString()
      );
      return;
    ...
    if (!okDom) {
      YAHOO.log(
        'Could not instantiate DataTable due to an invalid DOM element',
        'error',
        this.toString()
      );
      return;
  ...
  lang.augmentObject(DT, {
    ...
    formatButton: function (el, oRecord, oColumn, oData, oDataTable) {
      ...
      //else {
      el.innerHTML =
        '<button type="button" class="' + DT.CLASS_BUTTON + '">' + sValue + '</button>';
      //}
    },
    ...
    formatCheckbox: function (el, oRecord, oColumn, oData, oDataTable) {
      ...
      el.innerHTML = '<input type="checkbox"' + bChecked + ' class="' + DT.CLASS_CHECKBOX + '" />';
    },
    ...
    formatCurrency: function (el, oRecord, oColumn, oData, oDataTable) {
      ...
      el.innerHTML = util.Number.format(
        oData,
        oColumn.currencyOptions || oDT.get('currencyOptions')
      );
    },
    ...
    formatDropdown: function (el, oRecord, oColumn, oData, oDataTable) {
      ...
      if (selectEl) {
        ...
        // Selected value is our only option
          ...
          selectEl.innerHTML =
            '<option selected value="' + selectedValue + '">' + selectedValue + '</option>';
        }
      } else {
    ...
    validateNumber: function (oData) {
      ...
      if (lang.isNumber(number)) {
        ...
        YAHOO.log(
          'Could not validate data ' + lang.dump(oData) + ' to type Number',
          'warn',
          this.toString()
        );
        return undefined;
    ...
    initAttributes: function (oConfigs) {
      ...
      this.setAttributeConfig('sortedBy', {
        ...
        method: function (oNewSortedBy) {
          ...
          var oOldColumn, nOldColumnKeyIndex, oNewColumn, nNewColumnKeyIndex;
          ...
          if (this._elThead) {
            ...
            if (oNewSortedBy) {
              oNewColumn = oNewSortedBy.column
                ? oNewSortedBy.column
                : this._oColumnSet.getColumn(oNewSortedBy.key);
              ...
              if (oNewSortedBy.dir && (oNewSortedBy.dir == 'asc' || oNewSortedBy.dir == 'desc')) {
                var newClass = oNewSortedBy.dir == 'desc' ? DT.CLASS_DESC : DT.CLASS_ASC;
    ...
    */
      ...
      this.setAttributeConfig('MSG_SORTASC', {
        ...
        method: function (sParam) {
          if (this._elThead) {
            for (
              var i = 0, allKeys = this.getColumnSet().keys, len = allKeys.length;
              i < len;
              i++
            ) {
    ...
    _repaintOpera: ua.opera
      ? function () {
          if (ua.opera) {
            ...
            document.documentElement.className = YAHOO.lang.trim(
              document.documentElement.className
            );
          }
        }
    ...
    _initColumnSet: function (aColumnDefs) {
      ...
      if (this._oColumnSet) {
        ...
        for (i = 0, len = this._oColumnSet.keys.length; i < len; i++) {
          ...
          DT._oDynStyles[
            '.' + this.getId() + '-col-' + oColumn.getSanitizedKey() + ' .' + DT.CLASS_LINER
          ] = undefined;
          if (oColumn.editor && oColumn.editor.unsubscribeAll) {
            // Backward compatibility
      ...
      // Backward compatibility
        ...
        YAHOO.log(
          "DataTable's constructor now requires an array" +
      ...
      for (i = 0, len = allKeys.length; i < len; i++) {
        oColumn = allKeys[i];
        if (oColumn.editor && oColumn.editor.subscribe) {
      ...
      // Backward compatibility
        ...
        if (tmpContainer.hasChildNodes()) {
          ...
          if (tmpTable) {
            ...
            YAHOO.log(
              'Null DataSource for progressive enhancement from' + ' markup has been deprecated',
              'warn',
              this.toString()
            );
          }
    ...
    _initTableEl: function (elContainer) {
      if (elContainer) {
        ...
        Ev.delegate(
          this._elTable,
          'mouseenter',
          this._onTableMouseover,
          'thead .' + DT.CLASS_LABEL,
          this
        );
        Ev.delegate(
          this._elTable,
    ...
    _initColgroupEl: function (elTable) {
      if (elTable) {
        ...
        // Create COLGROUP
        var elColgroup = elTable.insertBefore(
    ...
    _reorderColgroupColEl: function (aKeyIndexes, newIndex) {
      if (
        lang.isArray(aKeyIndexes) &&
        lang.isNumber(newIndex) &&
        this._elColgroup &&
        this._elColgroup.childNodes.length > aKeyIndexes[aKeyIndexes.length - 1]
      ) {
        var i,
          tmpCols = [];
        // Remove COL
    ...
    _initTheadEl: function (elTable) {
      ...
      if (elTable) {
        ...
        YAHOO.log(
          'TH cells for ' + this._oColumnSet.keys.length + ' keys created',
          'info',
          this.toString()
        );
    ...
    _initThEl: function (elTh, oColumn) {
      ...
      if (oColumn.width) {
        // Validate minWidth
        var nWidth =
          oColumn.minWidth && oColumn.width < oColumn.minWidth ? oColumn.minWidth : oColumn.width;
        // ...for fallback cases
    ...
    _setSelections: function () {
      ...
      if (allSelectedRows.length > 0 || allSelectedCells.length > 0) {
        ...
        for (i = 0; i < allSelectedCells.length; i++) {
          ...
          if (el) {
            Dom.addClass(
              el.childNodes[oColumnSet.getColumn(allSelectedCells[i].columnKey).getKeyIndex()],
              DT.CLASS_SELECTED
            );
          }
        }
      ...
      if (!Dom.isAncestor(oSelf._elContainer, elTarget)) {
        ...
        if (oSelf._oCellEditor) {
          if (oSelf._oCellEditor.getContainerEl) {
            ...
            if (!Dom.isAncestor(elContainer, elTarget) && elContainer.id !== elTarget.id) {
          ...
          // Backward Compatibility
            ...
            if (
              !Dom.isAncestor(oSelf._oCellEditor.container, elTarget) &&
        ...
        switch (elTag) {
          ...
          case 'span':
            if (Dom.hasClass(elTarget, DT.CLASS_LABEL)) {
              bKeepBubbling = oSelf.fireEvent('theadLabelMouseoverEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerLabelMouseoverEvent', {
                target: elTarget,
                event: e,
              });
            }
            break;
          case 'th':
            bKeepBubbling = oSelf.fireEvent('theadCellMouseoverEvent', {
              target: elTarget,
              event: e,
            });
            // Backward compatibility
            bKeepBubbling = oSelf.fireEvent('headerCellMouseoverEvent', {
              target: elTarget,
              event: e,
            });
            break;
          case 'tr':
            if (elTarget.parentNode.nodeName.toLowerCase() == 'thead') {
              bKeepBubbling = oSelf.fireEvent('theadRowMouseoverEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerRowMouseoverEvent', {
                target: elTarget,
                event: e,
              });
            } else {
    ...
    _onTableMouseout: function (e, origTarget, container, oSelf) {
      ...
      while (elTarget && elTag != 'table') {
        switch (elTag) {
          ...
          case 'span':
            if (Dom.hasClass(elTarget, DT.CLASS_LABEL)) {
              bKeepBubbling = oSelf.fireEvent('theadLabelMouseoutEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerLabelMouseoutEvent', {
                target: elTarget,
                event: e,
              });
            }
            break;
          case 'th':
            bKeepBubbling = oSelf.fireEvent('theadCellMouseoutEvent', {
              target: elTarget,
              event: e,
            });
            // Backward compatibility
            bKeepBubbling = oSelf.fireEvent('headerCellMouseoutEvent', {
              target: elTarget,
              event: e,
            });
            break;
          case 'tr':
            if (elTarget.parentNode.nodeName.toLowerCase() == 'thead') {
              bKeepBubbling = oSelf.fireEvent('theadRowMouseoutEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerRowMouseoutEvent', {
                target: elTarget,
                event: e,
              });
            } else {
    ...
    _onTableMousedown: function (e, oSelf) {
      ...
      while (elTarget && elTag != 'table') {
        switch (elTag) {
          ...
          case 'span':
            if (Dom.hasClass(elTarget, DT.CLASS_LABEL)) {
              bKeepBubbling = oSelf.fireEvent('theadLabelMousedownEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerLabelMousedownEvent', {
                target: elTarget,
                event: e,
              });
            }
            break;
          case 'th':
            bKeepBubbling = oSelf.fireEvent('theadCellMousedownEvent', {
              target: elTarget,
              event: e,
            });
            // Backward compatibility
            bKeepBubbling = oSelf.fireEvent('headerCellMousedownEvent', {
              target: elTarget,
              event: e,
            });
            break;
          case 'tr':
            if (elTarget.parentNode.nodeName.toLowerCase() == 'thead') {
              bKeepBubbling = oSelf.fireEvent('theadRowMousedownEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerRowMousedownEvent', {
                target: elTarget,
                event: e,
              });
            } else {
    ...
    _onTableMouseup: function (e, oSelf) {
      ...
      while (elTarget && elTag != 'table') {
        switch (elTag) {
          ...
          case 'span':
            if (Dom.hasClass(elTarget, DT.CLASS_LABEL)) {
              bKeepBubbling = oSelf.fireEvent('theadLabelMouseupEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerLabelMouseupEvent', {
                target: elTarget,
                event: e,
              });
            }
            break;
          case 'th':
            bKeepBubbling = oSelf.fireEvent('theadCellMouseupEvent', {
              target: elTarget,
              event: e,
            });
            // Backward compatibility
            bKeepBubbling = oSelf.fireEvent('headerCellMouseupEvent', {
              target: elTarget,
              event: e,
            });
            break;
          case 'tr':
            if (elTarget.parentNode.nodeName.toLowerCase() == 'thead') {
              bKeepBubbling = oSelf.fireEvent('theadRowMouseupEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerRowMouseupEvent', {
                target: elTarget,
                event: e,
              });
            } else {
    ...
    _onTableDblclick: function (e, oSelf) {
      ...
      while (elTarget && elTag != 'table') {
        switch (elTag) {
          ...
          case 'span':
            if (Dom.hasClass(elTarget, DT.CLASS_LABEL)) {
              bKeepBubbling = oSelf.fireEvent('theadLabelDblclickEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerLabelDblclickEvent', {
                target: elTarget,
                event: e,
              });
            }
            break;
          case 'th':
            bKeepBubbling = oSelf.fireEvent('theadCellDblclickEvent', {
              target: elTarget,
              event: e,
            });
            // Backward compatibility
            bKeepBubbling = oSelf.fireEvent('headerCellDblclickEvent', {
              target: elTarget,
              event: e,
            });
            break;
          case 'tr':
            if (elTarget.parentNode.nodeName.toLowerCase() == 'thead') {
              bKeepBubbling = oSelf.fireEvent('theadRowDblclickEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerRowDblclickEvent', {
                target: elTarget,
                event: e,
              });
            } else {
      ...
      while (elTarget && elTag != 'table') {
        switch (elTag) {
          ...
          case 'input':
            ...
            if (sType == 'checkbox') {
              bKeepBubbling = oSelf.fireEvent('theadCheckboxClickEvent', {
                target: elTarget,
                event: e,
              });
            } else if (sType == 'radio') {
              bKeepBubbling = oSelf.fireEvent('theadRadioClickEvent', {
                target: elTarget,
                event: e,
              });
            } else if (
              ...
              if (!elTarget.disabled) {
                bKeepBubbling = oSelf.fireEvent('theadButtonClickEvent', {
                  target: elTarget,
                  event: e,
                });
              } else {
          ...
          case 'span':
            if (Dom.hasClass(elTarget, DT.CLASS_LABEL)) {
              bKeepBubbling = oSelf.fireEvent('theadLabelClickEvent', {
                target: elTarget,
                event: e,
              });
              // Backward compatibility
              bKeepBubbling = oSelf.fireEvent('headerLabelClickEvent', {
                target: elTarget,
                event: e,
              });
            }
    ...
    getState: function () {
      return {
        totalRecords: this.get('paginator')
          ? this.get('paginator').get('totalRecords')
          : this._oRecordSet.getLength(),
    ...
    getTdEl: function (cell) {
      ...
      if (el && el.ownerDocument == document) {
        ...
        if (
          elCell &&
          (elCell.parentNode.parentNode == this._elTbody ||
    ...
    getNextTdEl: function (cell) {
      ...
      if (elCell) {
        ...
        if (elRow.cells && elRow.cells.length > 0 && nThisTdIndex < elRow.cells.length - 1) {
          ...
          elRow.childNodes &&
          elRow.childNodes.length > 0 &&
          nThisTdIndex < elRow.childNodes.length - 1
        ) {
    ...
    _getViewRecords: function () {
      ...
      var oPaginator = this.get('paginator');
      if (oPaginator) {
        return this._oRecordSet.getRecords(oPaginator.getStartIndex(), oPaginator.getRowsPerPage());
      }
          ...
          method: function (oArg) {
            if (this instanceof DT && this._sId) {
              var i = oArg.nCurrentRecord,
                endRecordIndex =
                  oArg.nCurrentRecord + oArg.nLoopLength > nRecordsLength
                    ? nRecordsLength
                    : oArg.nCurrentRecord + oArg.nLoopLength,
                elRow,
                nextSibling;
    ...
    getColumnSortDir: function (oColumn, oSortedBy) {
      ...
      var sortDir =
        oColumn.sortOptions && oColumn.sortOptions.defaultDir
          ? oColumn.sortOptions.defaultDir
          : DT.CLASS_ASC;
      ...
      if (oColumn && oColumn instanceof YAHOO.widget.Column) {
        ...
        if (ok) {
          ...
          // Client-side sort
            ...
            var sortFnc =
              oColumn.sortOptions && lang.isFunction(oColumn.sortOptions.sortFunction)
            ...
            if (!bSorted || sDir || sortFnc) {
              ...
              var sField =
                oColumn.sortOptions && oColumn.sortOptions.field
                  ? oColumn.sortOptions.field
                  : oColumn.field;

              // Sort the Records
              this._oRecordSet.sortRecords(
                sortFnc,
                sSortDir == DT.CLASS_DESC ? true : false,
                sField
              );
            }
          ...
          YAHOO.log(
            'Column "' + oColumn.key + '" sorted "' + sSortDir + '"',
            'info',
            this.toString()
          );
          return;
    ...
    setColumnWidth: function (oColumn, nWidth) {
      ...
      if (oColumn) {
        ...
        if (lang.isNumber(nWidth)) {
          ...
          YAHOO.log(
            'Set width of Column ' + oColumn + ' to ' + nWidth + 'px',
            'info',
            this.toString()
          );
        }
        ...
        YAHOO.log(
          'Could not set width of Column ' + oColumn + ' to ' + nWidth + 'px',
          'warn',
          this.toString()
        );
      }
    ...
    _setColumnWidth: function (oColumn, sWidth, sOverflow) {
      if (oColumn && oColumn.getKeyIndex() !== null) {
        ...
        YAHOO.log(
          'Could not set width of unknown Column ' + oColumn + ' to ' + sWidth,
          'warn',
          this.toString()
        );
      }
      ...
      if (s) {
        ...
        if (!rule) {
          if (s.styleSheet && s.styleSheet.addRule) {
            ...
            s.sheet.insertRule(
              sClassname + ' {overflow:' + sOverflow + ';width:' + sWidth + ';}',
    ...
    _setColumnWidthDynFunction: function (oColumn, sWidth, sOverflow) {
      ...
      if (!this._aDynFunctions[rowslen]) {
        ...
        resizerDef[k] = 'sWidth;';
        this._aDynFunctions[rowslen] = new Function(
          'oColumn',
          'sWidth',
          'sOverflow',
          resizerDef.join('')
      ...
      if (oColumn && !oColumn.hidden && !oColumn.width && oColumn.getKeyIndex() !== null) {
        ...
        if (oColumn.minWidth > 0 && elThLiner.offsetWidth < oColumn.minWidth) {
          elColgroupClone.childNodes[oColumn.getKeyIndex()].style.width =
            ...
            (parseInt(Dom.getStyle(elThLiner, 'paddingRight'), 10) | 0) +
            'px';
    ...
    hideColumn: function (oColumn) {
      ...
      if (oColumn && !oColumn.hidden && oColumn.getTreeIndex() !== null) {
        ...
        YAHOO.log(
          'Could not hide Column "' +
            lang.dump(oColumn) +
            '". Only non-nested Columns can be hidden',
          'warn',
          this.toString()
        );
      }
    ...
    showColumn: function (oColumn) {
      ...
      if (oColumn && oColumn.hidden && oColumn.getTreeIndex() !== null) {
        ...
        YAHOO.log(
          'Could not show Column "' +
            lang.dump(oColumn) +
            '". Only non-nested Columns can be shown',
          'warn',
          this.toString()
        );
      }
    ...
    removeColumn: function (oColumn) {
      ...
      YAHOO.log(
        'Could not remove Column "' + oColumn.key + '". Only non-nested Columns can be removed',
        'warn',
        this.toString()
      );
    },
    ...
    insertColumn: function (oColumn, index) {
      ...
      if (oColumn instanceof YAHOO.widget.Column) {
        ...
        YAHOO.log(
          'Could not insert Column "' + oColumn + '" due to invalid argument',
          'warn',
          this.toString()
        );
        return;
      ...
      if (descKeyIndexes.length > 0) {
        ...
        if (allRows.length > 0) {
          ...
          for (i = 0, len = descKeyIndexes.length; i < len; i++) {
            ...
            elTdTemplate = this._formatTdEl(
              this._oColumnSet.keys[thisKeyIndex],
              elTdTemplate,
              thisKeyIndex,
              thisKeyIndex === this._oColumnSet.keys.length - 1
            );
          ...
          this._oChainRender.add({
              ...
              if (this instanceof DT && this._sId) {
                ...
                for (; i < len; ++i) {
                  ...
                  for (j = descKeyIndexes.length - 1; j > -1; j--) {
                    allRows[i].insertBefore(
                      oArg.aTdTemplates[descKeyIndexes[j]].cloneNode(true),
                      nextSibling
                    );
                  }
                }
            ...
            argument: {
              nCurrentRow: 0,
              aTdTemplates: aTdTemplates,
              descKeyIndexes: descKeyIndexes,
            },
            scope: this,
            timeout: loopN > 0 ? 0 : -1,
          });
        ...
        YAHOO.log(
          'Column "' + oColumn.key + '" inserted into index ' + index,
          'info',
          this.toString()
        );
        return oNewColumn;
    ...
    reorderColumn: function (oColumn, index) {
      ...
      YAHOO.log(
        'Could not reorder Column "' + oColumn.key + '". Only non-nested Columns can be reordered',
        'warn',
        this.toString()
      );
    },
        ...
        if (oColumn.getKeyIndex() !== null) {
          ...
          oChainRender.add({
            method: function (oArg) {
              if (
                this instanceof DT &&
                this._sId &&
                allRows[oArg.rowIndex] &&
                allRows[oArg.rowIndex].cells[oArg.cellIndex]
              ) {
          ...
          YAHOO.log(
            'Could not select Column "' +
              oColumn.key +
              '". Only non-nested Columns can be selected',
            'warn',
            this.toString()
          );
        }
    ...
    unselectColumn: function (oColumn) {
      ...
      if (oColumn && oColumn.selected) {
        ...
        if (oColumn.getKeyIndex() !== null) {
          ...
          YAHOO.log(
            'Could not unselect Column "' +
              oColumn.key +
              '". Only non-nested Columns can be unselected',
            'warn',
            this.toString()
          );
        }
    ...
    highlightColumn: function (column) {
      ...
      if (oColumn && oColumn.getKeyIndex() !== null) {
        ...
        YAHOO.log(
          'Could not highlight Column "' +
            oColumn.key +
            '". Only non-nested Columns can be highlighted',
          'warn',
          this.toString()
        );
      }
    ...
    unhighlightColumn: function (column) {
      ...
      if (oColumn && oColumn.getKeyIndex() !== null) {
        ...
        YAHOO.log(
          'Could not unhighlight Column "' +
            oColumn.key +
            '". Only non-nested Columns can be unhighlighted',
          'warn',
          this.toString()
        );
      }
    ...
    addRow: function (oData, index) {
      if (lang.isNumber(index) && (index < 0 || index > this._oRecordSet.getLength())) {
        YAHOO.log(
          'Could not add row at index ' + index + ' with ' + lang.dump(oData),
          'warn',
          this.toString()
        );
        return;
      ...
      if (oData && lang.isObject(oData)) {
        ...
        if (oRecord) {
          ...
          if (oPaginator) {
            ...
            YAHOO.log(
              'Added a row for Record ' +
                YAHOO.lang.dump(oRecord) +
                ' at RecordSet index ' +
                recIndex,
              'info',
              this.toString()
              ...
              this._oChainRender.add({
                method: function (oArg) {
                  if (this instanceof DT && this._sId) {
                    ...
                    if (elNewTr) {
                      var elNext = this._elTbody.rows[recIndex]
    ...
    addRows: function (aData, index) {
      if (lang.isNumber(index) && (index < 0 || index > this._oRecordSet.getLength())) {
        YAHOO.log(
          'Could not add rows at index ' + index + ' with ' + lang.dump(aData),
          'warn',
          this.toString()
        );
        return;
          ...
          if (oPaginator) {
            ...
            YAHOO.log(
              'Added ' +
                aRecords.length +
                ' rows at index ' +
                this._oRecordSet.getRecordIndex(aRecords[0]) +
                ' with data ' +
                lang.dump(aData),
              'info',
              this.toString()
    ...
    updateRow: function (row, oData) {
                ...
                if (this instanceof DT && this._sId) {
                  ...
                  YAHOO.log(
                    'DataTable row updated: Record ID = ' +
                      updatedRecord.getId() +
                      ', Record index = ' +
                      this.getRecordIndex(updatedRecord) +
                      ', page row index = ' +
                      this.getTrIndex(updatedRecord),
                    'info',
                    this.toString()
                  );
                }
      ...
      YAHOO.log(
        'Could not update row ' + row + ' with the data : ' + lang.dump(oData),
        'warn',
        this.toString()
      );
      return;
    ...
    updateRows: function (startrow, aData) {
                ...
                method: function (oArg) {
                  if (this instanceof DT && this._sId) {
                    var aRecords = oArg.aRecords,
                      ...
                      len =
                        loopN > 0
                          ? Math.min(i + loopN, startIndex + aRecords.length)
                          : startIndex + aRecords.length;
              ...
              this._oChainRender.add({
                method: function (oArg) {
                  ...
                  this.fireEvent('rowsAddEvent', {
                    newRecords: aNewRecords,
                    oldRecords: aOldRecords,
                  });
                  YAHOO.log(
                    'Added ' +
                      aNewRecords.length +
      ...
      YAHOO.log(
        'Could not update rows at ' + startrow + ' with ' + lang.dump(aData),
        'warn',
        this.toString()
      );
    },
      ...
      if (lang.isNumber(nRecordIndex)) {
        ...
        if (oRecord) {
          ...
          for (var j = tracker.length - 1; j > -1; j--) {
            if (
              (lang.isString(tracker[j]) && tracker[j] === sRecordId) ||
              ...
              this._oChainRender.add({
                method: function () {
                  if (this instanceof DT && this._sId) {
                    ...
                    YAHOO.log(
                      'Deleted row with data ' +
                        YAHOO.lang.dump(oData) +
                        ' at RecordSet index ' +
                        nRecordIndex +
                        ' and page row index ' +
                        nTrIndex,
              ...
              this._oChainRender.add({
                method: function (oArg) {
                  if (this instanceof DT && this._sId) {
                    ...
                    YAHOO.log(
                      'DataTable ' + count + ' rows deleted starting at index ' + lowIndex,
                      'info',
                      this.toString()
                    );
                  }
    ...
    formatCell: function (elLiner, oRecord, oColumn) {
      ...
      if (oRecord && oColumn) {
        ...
        var fnFormatter =
          typeof oColumn.formatter === 'function'
            ? oColumn.formatter
            : DT.Formatter[oColumn.formatter + ''] || DT.Formatter.defaultFormatter;
    ...
    onPaginatorChangeRequest: function (oPaginatorState) {
      ...
      if (ok) {
        ...
        YAHOO.log(
          'Could not change Paginator value "' + oPaginatorState + '"',
          'warn',
          this.toString()
        );
      }
    ...
    _handleStandardSelectionByMouse: function (oArgs) {
      ...
      if (elTargetRow) {
        ...
        var bCTRL =
          e.ctrlKey || (navigator.userAgent.toLowerCase().indexOf('mac') != -1 && e.metaKey);
    ...
    isSelected: function (o) {
      if (o && o.ownerDocument == document) {
        return (
          Dom.hasClass(this.getTdEl(o), DT.CLASS_SELECTED) ||
          Dom.hasClass(this.getTrEl(o), DT.CLASS_SELECTED)
        );
      } else {
          ...
          // Backward compatibility
            ...
            if (oRecord && oColumn) {
              ...
              oCellEditor.validator =
                oColumn.editorOptions && lang.isFunction(oColumn.editorOptions.validator)
                  ? oColumn.editorOptions.validator
                  : null;
      ...
      if (this._oCellEditor) {
        ...
        // Backward compatibility
          ...
          if (this._oCellEditor.validator) {
            newData = this._oCellEditor.value = this._oCellEditor.validator.call(
              this,
              newData,
              oldData,
              this._oCellEditor
            ...
            if (newData === null) {
              ...
              YAHOO.log(
                'Could not save Cell Editor input due to invalid data ' + lang.dump(newData),
                'warn',
                this.toString()
              );
              return;
          ...
          this._oRecordSet.updateRecordValue(
            this._oCellEditor.record,
            this._oCellEditor.column.key,
            this._oCellEditor.value
          );
          // Update the UI
          this.formatCell(
            this._oCellEditor.cell.firstChild,
            this._oCellEditor.record,
            this._oCellEditor.column
          );
    ...
    showCellEditorBtns: function (elContainer) {
      ...
      YAHOO.log(
        'The method showCellEditorBtns() has been deprecated.' +
          ' Please use the CellEditor class.',
        'warn',
        this.toString()
      );
    },
    ...
    resetCellEditor: function () {
      ...
      YAHOO.log(
        'The method resetCellEditor() has been deprecated.' + ' Please use the CellEditor class.',
        'warn',
        this.toString()
      );
    },
    ...
    getBody: function () {
      // Backward compatibility
      YAHOO.log(
        'The method getBody() has been deprecated' + ' in favor of getTbodyEl()',
        'warn',
        this.toString()
      );
    ...
    getCell: function (index) {
      // Backward compatibility
      YAHOO.log(
        'The method getCell() has been deprecated' + ' in favor of getTdEl()',
        'warn',
        this.toString()
      );
    ...
    getRow: function (index) {
      // Backward compatibility
      YAHOO.log(
        'The method getRow() has been deprecated' + ' in favor of getTrEl()',
        'warn',
        this.toString()
      );
    ...
    refreshView: function () {
      // Backward compatibility
      YAHOO.log(
        'The method refreshView() has been deprecated' + ' in favor of render()',
        'warn',
        this.toString()
      );
      this.render();
    ...
    select: function (els) {
      // Backward compatibility
      YAHOO.log(
        'The method select() has been deprecated' + ' in favor of selectRow()',
        'warn',
        this.toString()
      );
      if (!lang.isArray(els)) {
    ...
    onEventEditCell: function (oArgs) {
      // Backward compatibility
      YAHOO.log(
        'The method onEventEditCell() has been deprecated' + ' in favor of onEventShowCellEditor()',
        'warn',
        this.toString()
      );
    ...
    _syncColWidths: function () {
      // Backward compatibility
      YAHOO.log(
        'The method _syncColWidths() has been deprecated' + ' in favor of validateColumnWidths()',
        'warn',
        this.toString()
      );
      this.validateColumnWidths();
...
(function () {
  ...
  widget.ScrollingDataTable = function (elContainer, aColumnDefs, oDataSource, oConfigs) {
    ...
    widget.ScrollingDataTable.superclass.constructor.call(
      this,
      elContainer,
      aColumnDefs,
      oDataSource,
  ...
  lang.extend(SDT, DT, {
    ...
    _initDomElements: function (elContainer) {
      ...
      if (
        ...
        YAHOO.log(
          'Could not instantiate DataTable due to an invalid DOM elements',
          'error',
          this.toString()
        );
        return false;
    ...
    _initBdTheadEl: function (elTable) {
      if (elTable) {
        ...
        YAHOO.log(
          'Accessibility TH cells for ' + this._oColumnSet.keys.length + ' keys created',
          'info',
          this.toString()
        );
      }
    ...
    _initTbodyEl: function (elTable) {
      ...
      elTable.style.marginTop =
        this._elTbody.offsetTop > 0 ? '-' + this._elTbody.offsetTop + 'px' : 0;
    },
    ...
    _validateColumnWidth: function (oColumn, elTd) {
      ...
      if (!oColumn.width && !oColumn.hidden) {
        ...
        if (elTh.offsetWidth !== elTd.offsetWidth) {
          var elWider =
            elTh.offsetWidth > elTd.offsetWidth ? oColumn.getThLinerEl() : elTd.firstChild;
    ...
    disable: function () {
      ...
      elMask.style.height =
        this._elHdContainer.offsetHeight + this._elBdContainer.offsetHeight + 'px';
      elMask.style.display = '';
      this.fireEvent('disableEvent');
    },
    ...
    _onColumnChange: function (oArg) {
      ...
      var oColumn = oArg.column ? oArg.column : oArg.editor ? oArg.editor.column : null;
      this._storeScrollPositions();
      this.validateColumnWidths(oColumn);
    },
...
(function () {
  ...
  BCE.prototype = {
    ...
    _initContainerEl: function () {
      ...
      this.className = lang.isArray(this.className)
        ? this.className
        : this.className
        ? [this.className]
        : [];
    ...
    save: function () {
      ...
      if (this.validator) {
        ...
        if (validValue === undefined) {
          ...
          YAHOO.log(
            'Could not save Cell Editor input due to invalid data ' + lang.dump(inputValue),
            'warn',
            this.toString()
          );
          return;
      ...
      var finishSave = function (bSuccess, oNewValue) {
        ...
        if (bSuccess) {
          ...
          YAHOO.log(
            'Could not save Cell Editor input ' + lang.dump(oNewValue),
            'warn',
            oSelf.toString()
          );
        }
  ...
  widget.CheckboxCellEditor = function (oConfigs) {
    ...
    YAHOO.widget.BaseCellEditor._nCount++;
    widget.CheckboxCellEditor.superclass.constructor.call(
      this,
      oConfigs.type || 'checkbox',
      oConfigs
    );
  };
  ...
  widget.DropdownCellEditor = function (oConfigs) {
    ...
    YAHOO.widget.BaseCellEditor._nCount++;
    widget.DropdownCellEditor.superclass.constructor.call(
      this,
    ...
    renderForm: function () {
      if (lang.isArray(this.radioOptions)) {
        ...
        for (var i = 0, len = this.radioOptions.length; i < len; i++) {
          ...
          radioValue = lang.isValue(radioOption.value) ? radioOption.value : radioOption;
          ...
          elLabel.innerHTML = lang.isValue(radioOption.label) ? radioOption.label : radioOption;
        }
  ...
  widget.TextareaCellEditor = function (oConfigs) {
    ...
    YAHOO.widget.BaseCellEditor._nCount++;
    widget.TextareaCellEditor.superclass.constructor.call(
      this,
      oConfigs.type || 'textarea',
      oConfigs
    );
  };
  ...
  widget.TextboxCellEditor = function (oConfigs) {
    ...
    YAHOO.widget.BaseCellEditor._nCount++;
    widget.TextboxCellEditor.superclass.constructor.call(
      this,
      oConfigs.type || 'textbox',
      oConfigs
    );
  };
