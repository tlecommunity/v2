...
YAHOO.tool.Profiler = (function () {
  ...
  function saveDataPoint(name, duration) {
    ...
    if (functionData.calls > 1) {
      functionData.avg =
        (functionData.avg * (functionData.calls - 1) + duration) / functionData.calls;
  ...
  return {
    ...
    start: function (name) {
      if (container[name]) {
        throw new Error(
          "Cannot use '" + name + "' for profiling through start(), name is already in use."
        );
      } else {
        if (!report[name]) {
          createReport(name);
    ...
    unregisterFunction: function (name, unregisterPrototype) {
      if (lang.isFunction(container[name])) {
        ...
        var owner = container[name].__yuiOwner,
          funcName = container[name].__yuiFuncName;
