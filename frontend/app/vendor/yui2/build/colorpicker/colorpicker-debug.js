...
(function () {
  ...
  YAHOO.extend(ColorPicker, YAHOO.util.Element, {
    ...
    _getCommand: function (e) {
      ...
      } else if ('8, 9, 13, 27, 37, 39'.indexOf(c) > -1 || e.ctrlKey || e.metaKey) {
        // special chars
        return 5;
      } else {
     ...
     */
    _updateRGB: function () {
      var rgb = [this.get(this.OPT.RED), this.get(this.OPT.GREEN), this.get(this.OPT.BLUE)];
    ...
    _initSliders: function () {
      ...
      this.pickerSlider = Slider.getSliderRegion(
        ...
        this.getElement(ID.PICKER_THUMB),
        0,
        size,
        0,
        size
    ...
    _bindUI: function () {
      ...
      Event.on(
        this.getElement(ID.WEBSAFE_SWATCH),
        'click',
        function (e) {
      ...
      Event.on(
        this.getElement(ID.CONTROLS_LABEL),
        'click',
        function (e) {
    ...
    initAttributes: function (attr) {
      ...
      this.setAttributeConfig(this.OPT.RGB, {
        ...
        method: function (rgb) {
          ...
          this.logger.log(
            sub('RGB value set to {rgb} (hsv: {hsv})', {
              hsv: hsv,
              rgb: rgb,
            })
          );
      ...
      this.setAttributeConfig(this.OPT.SHOW_CONTROLS, {
        ...
        method: function (on) {
          var el = Dom.getElementsByClassName('bd', 'div', this.getElement(this.ID.CONTROLS))[0];

          this._hideShowEl(el, on);

          this.getElement(this.ID.CONTROLS_LABEL).innerHTML = on
      ...
      this.setAttributeConfig(this.OPT.SHOW_HSV_CONTROLS, {
        value: lang.isBoolean(attr.showhsvcontrols) ? attr.showhsvcontrols : false,
        method: function (on) {
      ...
      this.setAttributeConfig(this.OPT.SHOW_HEX_CONTROLS, {
        value: lang.isBoolean(attr.showhexcontrols) ? attr.showhexcontrols : false,
        method: function (on) {
