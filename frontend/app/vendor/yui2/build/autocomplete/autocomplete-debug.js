...
YAHOO.widget.AutoComplete = function (elInput, elContainer, oDataSource, oConfigs) {
  if (elInput && elContainer && oDataSource) {
    ...
    if (oDataSource && YAHOO.lang.isFunction(oDataSource.sendRequest)) {
      ...
      YAHOO.log(
        'Could not instantiate AutoComplete due to an invalid DataSource',
        'error',
        this.toString()
      );
      return;
    ...
    if (oDataSource._aDeprecatedSchema) {
      ...
      if (YAHOO.lang.isArray(aDeprecatedSchema)) {
        if (
          oDataSource.responseType === YAHOO.util.DataSourceBase.TYPE_JSON ||
          oDataSource.responseType === YAHOO.util.DataSourceBase.TYPE_UNKNOWN
        ) {
          // Used to default to unknown
    ...
    if (YAHOO.util.Dom.inDocument(elInput)) {
      ...
      YAHOO.log(
        'Could not instantiate AutoComplete due to an invalid input element',
        'error',
        this.toString()
      );
      return;
    ...
    if (YAHOO.util.Dom.inDocument(elContainer)) {
      ...
      if (this._elContainer.style.display == 'none') {
        YAHOO.log(
          'The container may not display properly if display is set to "none" in CSS',
          'warn',
          this.toString()
        );
      }
      ...
      YAHOO.log(
        'Could not instantiate AutoComplete due to an invalid container element',
        'error',
        this.toString()
      );
      return;
...
YAHOO.widget.AutoComplete.prototype.getSubsetMatches = function (sQuery) {
  ...
  for (var i = sQuery.length; i >= this.minQueryLength; i--) {
    ...
    if (oCachedResponse) {
      YAHOO.log(
        'Found match for query subset "' + subQuery + '": ' + YAHOO.lang.dump(oCachedResponse),
        'info',
        this.toString()
      );
 ...
 * @return {Object} oParsedResponse or null.
 */
YAHOO.widget.AutoComplete.prototype.preparseRawResponse = function (
  oRequest,
  oFullResponse,
  oCallback
) {
  var nEnd =
    this.responseStripAfter !== '' && oFullResponse.indexOf
      ? oFullResponse.indexOf(this.responseStripAfter)
      : -1;
  if (nEnd != -1) {
...
YAHOO.widget.AutoComplete.prototype.filterResults = function (
  sQuery,
  oFullResponse,
  oParsedResponse,
  oCallback
  ...
  if (sQuery && sQuery !== '') {
    ...
    YAHOO.log(
      'Filtered ' +
        ...
        sQuery +
        '": ' +
        YAHOO.lang.dump(filteredResults),
      'info',
      this.toString()
    );
  } else {
 ...
 */
YAHOO.widget.AutoComplete.prototype.formatEscapedResult = function (
  oResultData,
  sQuery,
  sResultMatch
) {
...
YAHOO.widget.AutoComplete.prototype._initProps = function () {
  ...
  if (this.forceSelection && delimChar) {
    YAHOO.log(
      'The forceSelection feature has been enabled with delimChar defined.',
      'warn',
      this.toString()
    );
  }
...
YAHOO.widget.AutoComplete.prototype._isIgnoreKey = function (nKeyCode) {
  if (
    nKeyCode == 9 ||
    nKeyCode == 13 || // tab, enter
    nKeyCode == 16 ||
    nKeyCode == 17 || // shift, ctl
    ...
    if (this._bFocused) {
      ...
      var allResults = oResponse.results,
        ...
        sMatchKey = this.dataSource.responseSchema.fields
          ? this.dataSource.responseSchema.fields[0].key || this.dataSource.responseSchema.fields[0]
          : 0;
      ...
      YAHOO.log(
        'Container populated with ' + nItemsToShow + ' list items',
        'info',
        this.toString()
      );
      return;
 ...
 * @private
 */
YAHOO.widget.AutoComplete.prototype._doBeforeExpandContainer = function (
  elTextbox,
  elContainer,
  sQuery,
  aResults
...
YAHOO.widget.AutoComplete.prototype._typeAhead = function (elListItem, sQuery) {
  ...
  if (elTextbox.setSelectionRange || elTextbox.createTextRange) {
    ...
    this._nTypeAheadDelayID = setTimeout(function () {
      ...
      YAHOO.log(
        'Typeahead occured with prefill string "' + sPrefill + '"',
        'info',
        oSelf.toString()
      );
  ...
  if (this._bContainerOpen) {
    ...
    if (scrollOn && nNewItemIndex > -1 && nNewItemIndex < this._nDisplayedItems) {
      ...
      if (nKeyCode == 40) {
        ...
        if (
          elNewListItem.offsetTop + elNewListItem.offsetHeight >
          elContent.scrollTop + elContent.offsetHeight
        ) {
          // Set bottom of scroll area to bottom of selected item
          elContent.scrollTop =
            elNewListItem.offsetTop + elNewListItem.offsetHeight - elContent.offsetHeight;
        }
...
YAHOO.widget.AutoComplete.prototype._onTextboxKeyDown = function (v, oSelf) {
  ...
  switch (nKeyCode) {
    case 9: // tab
      if (
        (!YAHOO.env.ua.opera && navigator.userAgent.toLowerCase().indexOf('mac') == -1) ||
...
YAHOO.widget.AutoComplete.prototype._onTextboxKeyPress = function (v, oSelf) {
  ...
  if (
    YAHOO.env.ua.opera ||
    (navigator.userAgent.toLowerCase().indexOf('mac') != -1 && YAHOO.env.ua.webkit < 420)
  ) {
    switch (nKeyCode) {
      case 9: // tab
        // select an item or clear out
