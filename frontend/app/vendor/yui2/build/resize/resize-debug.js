  ...
  YAHOO.extend(Resize, YAHOO.util.Element, {
    ...
    _createWrap: function () {
      ...
      if (this.get('wrap') === false) {
        switch (this.get('element').tagName.toLowerCase()) {
          ...
          case 'select':
            YAHOO.log(
              'Auto-wrapping the element (' + this.get('element').tagName.toLowerCase() + ')',
              'warn',
              'Resize'
            );
            this.set('wrap', true);
    ...
    _createHandles: function () {
      ...
      for (var i = 0; i < h.length; i++) {
        ...
        this._dds[h[i]] = new YAHOO.util.DragDrop(
          this._handles[h[i]],
          this.get('id') + '-handle-' + h,
          { useShim: this.get('useShim') }
        );
      ...
      if (this._resizeEvent && !Lang.isString(this._resizeEvent)) {
        ...
        if (this.get('status')) {
          ...
          this._status.innerHTML =
            ...
            diffW +
            '</em>';
          D.setXY(this._status, [
            Event.getPageX(this._resizeEvent) + 12,
            Event.getPageY(this._resizeEvent) + 12,
          ]);
        }
      }
    },
