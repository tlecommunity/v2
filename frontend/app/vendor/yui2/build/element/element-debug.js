...
YAHOO.util.Attribute.prototype = {
  ...
  setValue: function (value, silent) {
    ...
    if (this.readOnly || (this.writeOnce && this._written)) {
      YAHOO.log('setValue ' + name + ', ' + value + ' failed: read only', 'error', 'Attribute');
      return false; // write not allowed
    }

    if (this.validator && !this.validator.call(owner, value)) {
      YAHOO.log('setValue ' + name + ', ' + value + ' validation failed', 'error', 'Attribute');
  ...
  YAHOO.util.AttributeProvider.prototype = {
    ...
    getAttributeKeys: function () {
      ...
      for (key in this._configs) {
        if (Lang.hasOwnProperty(this._configs, key) && !Lang.isUndefined(this._configs[key])) {
  ...
  Element.prototype = {
    ...
    addListener: function (type, fn, obj, scope) {
      ...
      if (specialTypes[type] && !Event._createMouseDelegate) {
        YAHOO.log(
          'Using a ' + type + ' event requires the event-mouseenter module',
          'error',
          'Event'
        );
        return false;
