...
YAHOO.util.History = (function () {
    ...
    for (moduleName in _modules) {
      if (YAHOO.lang.hasOwnProperty(_modules, moduleName)) {
        ...
        if (!currentState || moduleObj.currentState !== currentState) {
          moduleObj.currentState =
            typeof currentState === 'undefined' ? moduleObj.initialState : currentState;
          moduleObj.onStateChange(_decode(moduleObj.currentState));
        }
      }
    }
  ...
  function _updateIFrame(fqstate) {
    ...
    html = '<html><body><div id="state">' + YAHOO.lang.escapeHTML(fqstate) + '</div></body></html>';
    ...
    if (parts.length > 1) {
      ...
      for (i = 0, len = initialStates.length; i < len; i++) {
        ...
        if (tokens.length === 2) {
          ...
          moduleObj = YAHOO.lang.hasOwnProperty(_modules, moduleName) && _modules[moduleName];
  ...
  return {
      ...
      if (
        ...
        (stateField.tagName.toUpperCase() !== 'TEXTAREA' &&
          (stateField.tagName.toUpperCase() !== 'INPUT' ||
            (stateField.type !== 'hidden' && stateField.type !== 'text')))
      ) {
        throw new Error('Missing or invalid argument');
      }
    ...
    getCurrentState: function (module) {
      ...
      moduleObj = YAHOO.lang.hasOwnProperty(_modules, module) && _modules[module];
