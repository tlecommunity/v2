...
(function () {
  ...
  function setAttributesFromSrcElement(p_oElement, p_oAttributes) {
    ...
    if (!('label' in p_oAttributes)) {
      ...
      sText = sSrcElementNodeName == 'INPUT' ? p_oElement.value : p_oElement.innerHTML;
  ...
  YAHOO.widget.Button = function (p_oElement, p_oAttributes) {
    ...
    if (arguments.length == 1 && !Lang.isString(p_oElement) && !p_oElement.nodeName) {
      ...
      YAHOO.log(
        'No source HTML element.  Building the button ' +
      ...
      if (Lang.isString(p_oElement)) {
        ...
        if (oElement) {
          ...
          if (!oConfig.element) {
            YAHOO.log(
              'Source element could not be used ' +
        ...
        if (!oConfig.attributes.id) {
          if (p_oElement.id) {
            ...
            YAHOO.log(
              "No value specified for the button's " +
  ...
  YAHOO.extend(YAHOO.widget.Button, YAHOO.util.Element, {
    ...
    _setMenu: function (p_oMenu) {
      ...
      if (Overlay) {
        ...
        if (p_oMenu && Menu && p_oMenu instanceof Menu) {
          ...
          if (oMenuElement) {
            if (
              (Menu && Dom.hasClass(oMenuElement, sMenuCSSClassName)) ||
          ...
          if (
            (Menu && Dom.hasClass(p_oMenu, sMenuCSSClassName)) ||
    ...
    _onMouseOver: function (p_oEvent) {
      ...
      if (sType === 'split') {
        ...
        nOptionRegionX = Dom.getX(oElement) + (oElement.offsetWidth - this.OPTION_AREA_WIDTH);
    ...
    _onDocumentMouseUp: function (p_oEvent) {
      ...
      if (sType == 'menu' || sType == 'split') {
        ...
        if (oTarget != oMenuElement && !Dom.isAncestor(oMenuElement, oTarget)) {
    ...
    _onKeyDown: function (p_oEvent) {
      ...
      if (this.get('type') == 'split' && this._isSplitButtonOptionKey(p_oEvent)) {
        this.fireEvent('option', p_oEvent);
    ...
    _onClick: function (p_oEvent) {
      ...
      switch (sType) {
        ...
        case 'split':
          if (this._nOptionRegionX > 0 && Event.getPageX(p_oEvent) > this._nOptionRegionX) {
            bReturnVal = false;
          } else {
            this._hideMenu();
            ...
            if (oSrcElement && oSrcElement.type == 'submit' && p_oEvent.returnValue !== false) {
              this.submitForm();
            }
          }
    ...
    _onMenuRender: function (p_sType) {
      ...
      if (oSrcElement && oSrcElement.nodeName.toLowerCase() === 'select' && oSrcElement.value) {
    ...
    createHiddenFields: function () {
      ...
      if (oForm && !this.get('disabled')) {
        ...
        if (Menu && oMenu && oMenu instanceof Menu) {
          ...
          bMenuSrcElementIsSelect =
            oMenuSrcElement && oMenuSrcElement.nodeName.toUpperCase() == 'SELECT';

          if (oMenuItem) {
            oValue =
              oMenuItem.value === null || oMenuItem.value === ''
    ...
    init: function (p_oElement, p_oAttributes) {
      ...
      } else if (this.get('type') != 'link' && bElInDoc && oSrcElement && oSrcElement == oElement) {
    ...
    destroy: function () {
      ...
      aButtons = Dom.getElementsByClassName(sClass, this.NODE_NAME, oForm);

      if (Lang.isArray(aButtons) && aButtons.length === 0) {
        Event.removeListener(oForm, 'keypress', YAHOO.widget.Button.onFormKeyPress);
      }
          ...
          if (sId) {
            ...
            if (oButton) {
              ...
              if (!oButton.get('disabled')) {
                ...
                if (
                  !oYUISubmitButton &&
                  (oButton.get('type') == 'submit' || (oSrcElement && oSrcElement.type == 'submit'))
                ) {
    ...
    if (arguments.length == 1 && !Lang.isString(p_oElement) && !p_oElement.nodeName) {
      if (!p_oElement.id) {
        ...
        YAHOO.log(
          'No value specified for the button group\'s "id"' +
            ' attribute. Setting button group id to "' +
            sId +
            '".',
          'info'
        );
      ...
      if (oElement) {
        if (oElement.nodeName.toUpperCase() == this.NODE_NAME) {
          this.logger = new YAHOO.widget.LogWriter('ButtonGroup ' + p_oElement);
      ...
      if (sNodeName && sNodeName == this.NODE_NAME) {
        if (!p_oElement.id) {
          ...
          YAHOO.log(
            "No value specified for the button group's" +
        ...
        this.logger = new YAHOO.widget.LogWriter('ButtonGroup ' + p_oElement.id);
  ...
  YAHOO.extend(YAHOO.widget.ButtonGroup, YAHOO.util.Element, {
    ...
    _onKeyDown: function (p_oEvent) {
      ...
      if (nCharCode == 37 || nCharCode == 38) {
        nIndex = oButton.index === 0 ? this._buttons.length - 1 : oButton.index - 1;
      } else if (nCharCode == 39 || nCharCode == 40) {
        nIndex = oButton.index === this._buttons.length - 1 ? 0 : oButton.index + 1;
      }
    ...
    init: function (p_oElement, p_oAttributes) {
      ...
      this.logger.log(
        'Searching for child nodes with the class name ' + sClass + ' to add to the button group.'
      );
      ...
      if (aButtons.length > 0) {
        this.logger.log(
          'Found ' +
            aButtons.length +
            ' child nodes with the class name ' +
            sClass +
