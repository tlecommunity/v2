    ...
    d.prototype = {
      ...
      removeItem: function (j) {
        ...
        if (i.hasKey(j)) {
          ...
          i.fireEvent(
            d.CE_CHANGE,
            new f.StorageEvent(i, j, h, null, f.StorageEvent.TYPE_REMOVE_ITEM)
          );
        } else {
        }
      },
          ...
          if (i._setItem(j, i._createValue(k))) {
            i.fireEvent(
              ...
              new f.StorageEvent(
                ...
                h,
                k,
                i.hasKey(j) ? f.StorageEvent.TYPE_UPDATE_ITEM : f.StorageEvent.TYPE_ADD_ITEM
              )
            );
          } else {
            throw (
...
(function () {
  ...
  YAHOO.register('StorageManager', h.SWFStore, { version: '@VERSION@', build: '@BUILD@' });
})();
(function () {
...
(function () {
  ...
  YAHOO.lang.extend(a.StorageEngineKeyed, a.Storage, {
    _keys: null,
    _keyMap: null,
    _addKey: function (b) {
...
(function () {
  var h = YAHOO.util,
    ...
    c = function (l, o) {
      ...
      if (!i) {
        b.execute('BEGIN');
        b.execute(
          'DELETE FROM ' + g + ' WHERE location="' + f(h.StorageManager.LOCATION_SESSION) + '"'
        );
        b.execute('COMMIT');
      }
  ...
  d.extend(c, h.StorageEngineKeyed, {
    _clear: function () {
    ...
    _removeItem: function (i) {
      ...
      b.execute('BEGIN');
      b.execute(
        'DELETE FROM ' + g + ' WHERE key="' + f(i) + '" AND location="' + f(this._location) + '"'
      );
      b.execute('COMMIT');
    },
...
(function () {
  var b = YAHOO,
    ...
    c = 215,
    h = 138,
    d = new RegExp('^(' + j.LOCATION_SESSION + '|' + j.LOCATION_LOCAL + ')'),
    e = null,
    k = function (m, n) {
      ...
      if (!e) {
        ...
        if (!n.containerID) {
          var o = document.getElementsByTagName('body')[0],
            m = o.appendChild(document.createElement('div'));
    ...
    l = function (m, p) {
      ...
      var n = function () {
        o._swf = e._swf;
        e.initialized = true;
        var s = j.LOCATION_SESSION === o._location,
          r = i.Cookie.get('sessionKey' + l.ENGINE_NAME),
          u,
          t,
          q;
  ...
  g.extend(l, i.StorageEngineKeyed, {
    _swf: null,
    _clear: function () {
      ...
      if (e.callSWF('setItem', [n, p])) {
        ...
        b.log(
          'attempting to show settings. are dimensions adequate? ' +
            e.callSWF('hasAdequateDimensions')
        );
