...
(function () {
  ...
  if (!Util.Storage) {
    _logOverwriteError = function (fxName) {
      Y.log(
        'Exception in YAHOO.util.Storage.?? - must be extended by a storage engine'
          .replace('??', fxName)
          .replace('??', this.getName ? this.getName() : 'Unknown'),
        'error'
      );
    };
    ...
    Storage.prototype = {
      ...
      key: function (nIndex) {
        ...
        throw (
          'INDEX_SIZE_ERR - Storage.setItem - The provided index (' + nIndex + ') is not available'
        );
      },
      ...
      removeItem: function (sKey) {
        ...
        if (that.hasKey(sKey)) {
          ...
          that.fireEvent(
            Storage.CE_CHANGE,
            new Util.StorageEvent(that, sKey, oOldValue, null, Util.StorageEvent.TYPE_REMOVE_ITEM)
          );
        } else {
      ...
      setItem: function (sKey, oData) {
        ...
        if (Lang.isString(sKey)) {
          ...
          if (that._setItem(sKey, that._createValue(oData))) {
            that.fireEvent(
              Storage.CE_CHANGE,
              new Util.StorageEvent(
                that,
                sKey,
                oOldValue,
                oData,
                that.hasKey(sKey)
                  ? Util.StorageEvent.TYPE_UPDATE_ITEM
                  : Util.StorageEvent.TYPE_ADD_ITEM
              )
            );
          } else {
...
(function () {
  ...
  Util.StorageManager = {
    ...
    register: function (engineConstructor) {
      if (
        Lang.isFunction(engineConstructor) &&
        Lang.isFunction(engineConstructor.isAvailable) &&
...
(function () {
  ...
  var Util = YAHOO.util,
    ...
    StorageEngineHTML5 = function (sLocation, oConf) {
      ...
      StorageEngineHTML5.superclass.constructor.call(
        that,
        sLocation,
        StorageEngineHTML5.ENGINE_NAME,
        oConf
      ); // not set, are cookies available
...
(function () {
  ...
  var Util = YAHOO.util,
    ...
    StorageEngineGears = function (sLocation, oConf) {
      ...
      StorageEngineGears.superclass.constructor.call(
        that,
        sLocation,
        StorageEngineGears.ENGINE_NAME,
        oConf
      );
      ...
      if (!_driver) {
        ...
        _driver.open(
          window.location.host.replace(/[\/\:\*\?"\<\>\|;,]/g, '') +
            '-' +
            StorageEngineGears.DATABASE
        );
        _driver.execute(
          'CREATE TABLE IF NOT EXISTS ' + TABLE_NAME + ' (key TEXT, location TEXT, value TEXT)'
        );
      }
      ...
      if (!sessionKey) {
        ...
        _driver.execute(
          'DELETE FROM ' +
            TABLE_NAME +
            ' WHERE location="' +
            eURI(Util.StorageManager.LOCATION_SESSION) +
            '"'
        );
      ...
      rs = _driver.execute(
        'SELECT key FROM ' + TABLE_NAME + ' WHERE location="' + eURI(that._location) + '"'
      );
      keyMap = {};
  ...
  Lang.extend(StorageEngineGears, Util.StorageEngineKeyed, {
    ...
    _clear: function () {
      ...
      _driver.execute('BEGIN');
      _driver.execute(
        'DELETE FROM ' + TABLE_NAME + ' WHERE location="' + eURI(this._location) + '"'
      );
      _driver.execute('COMMIT');
    },
...
(function () {
  ...
  var Y = YAHOO,
     ...
     */
    MINIMUM_HEIGHT = 138,
    RX_STORAGE_PREFIX = new RegExp(
      '^(' + StorageManager.LOCATION_SESSION + '|' + StorageManager.LOCATION_LOCAL + ')'
    ),
    // local variables
    _driver = null,
    /*
    ...
    StorageEngineSWF = function (sLocation, oConf) {
      var that = this;
      StorageEngineSWF.superclass.constructor.call(
        that,
        sLocation,
        StorageEngineSWF.ENGINE_NAME,
        oConf
      );
      ...
      var _onContentReady = function () {
        ...
        for (i = _driver.callSWF('getLength', []) - 1; 0 <= i; i -= 1) {
          ...
          isKeySessionStorage =
            isSessionStorage && -1 < key.indexOf(StorageManager.LOCATION_SESSION);
  ...
  Lang.extend(StorageEngineSWF, Util.StorageEngineKeyed, {
    ...
    _setItem: function (sKey, oData) {
      ...
      if (_driver.callSWF('setItem', [sLocationKey, oData])) {
        ...
        Y.log(
          'attempting to show settings. are dimensions adequate? ' +
            _driver.callSWF('hasAdequateDimensions')
        );
