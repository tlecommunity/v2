(function () {
  ...
  i.extend(r, YAHOO.util.Element, {
    ...
    _fixEdges: function () {
      ...
      var J = this.get(d),
        L = c.getElementsByClassName(q, undefined, J)[0],
        I = c.getElementsByClassName(l, undefined, J)[0],
        K = c.getElementsByClassName(k, undefined, J)[0],
        H = c.getElementsByClassName(g, undefined, J)[0],
        b = parseInt(c.getStyle(J, w), 10) - parseInt(c.getStyle(L, w), 10) + o;
      c.setStyle(K, w, b);
      c.setStyle(H, w, b);
    ...
    _recalculateConstants: function () {
      ...
      switch (this.get(x)) {
        ...
        case t:
          this._barSpace =
            parseInt(this.get(h), 10) -
            (parseInt(c.getStyle(b, 'marginLeft'), 10) || 0) -
            (parseInt(c.getStyle(b, 'marginRight'), 10) || 0);
          break;
        case G:
        case s:
          this._barSpace =
            parseInt(this.get(w), 10) -
            (parseInt(c.getStyle(b, 'marginTop'), 10) || 0) -
            (parseInt(c.getStyle(b, 'marginBottom'), 10) || 0);
          break;
      }
    ...
    _valueChange: function (J, H) {
      var I = this.get(a),
        b = Math.floor((J - this.get(m)) * this._barFactor);
      this._setAriaText(J);
      if (this._rendered) {
        if (I) {
          I.stop();
    ...
    _setAriaText: function (H) {
      var b = this.get('element'),
        I = i.substitute(this.get(v), { value: H, minValue: this.get(m), maxValue: this.get(y) });
