...
YAHOO.util.Assert = {
  ...
  isTypeOf: function (b, c, a) {
    if (typeof c != b) {
      throw new YAHOO.util.ComparisonFailure(
        this._formatMessage(a, 'Value should be of type ' + b + '.'),
        b,
        typeof c
      );
    }
...
YAHOO.util.ArrayAssert = {
  contains: function (e, d, b) {
    ...
    if (!c) {
      f.fail(
        f._formatMessage(b, 'Value ' + e + ' (' + typeof e + ') not found in array [' + d + '].')
      );
    }
  },
    ...
    for (var b = 0; b < d.length; b++) {
      if (d[b] === e) {
        YAHOO.util.Assert.areEqual(
          a,
          b,
          c || 'Value exists at index ' + b + ' but should be at index ' + a + '.'
        );
        return;
      }
    }
  ...
  itemsAreEquivalent: function (e, f, b, d) {
    ...
    for (var c = 0; c < a; c++) {
      if (!b(e[c], f[c])) {
        throw new YAHOO.util.ComparisonFailure(
          YAHOO.util.Assert._formatMessage(d, 'Values in position ' + c + ' are not equivalent.'),
          e[c],
          f[c]
        );
      }
    ...
    for (var b = d.length; b >= 0; b--) {
      if (d[b] === e) {
        f.areEqual(
          a,
          b,
          f._formatMessage(c, 'Value exists at index ' + b + ' but should be at index ' + a + '.')
        );
        return;
      }
    }
      ...
      switch (f.type) {
        case 'test':
          if (f.result != 'ignore') {
            ...
            if (f.result == 'fail') {
              e +=
                '<failure message="' + a(f.message) + '"><![CDATA[' + f.message + ']]></failure>';
            }
            e += '</testcase>';
          }
          break;
