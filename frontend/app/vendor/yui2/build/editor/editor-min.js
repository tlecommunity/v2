...
(function () {
  ...
  var B = function (F) {
    ...
    if (
      !(E instanceof YAHOO.widget.ToolbarButton) &&
      !(E instanceof YAHOO.widget.ToolbarButtonAdvanced)
    ) {
  ...
  YAHOO.extend(YAHOO.widget.Toolbar, YAHOO.util.Element, {
      ...
      for (var G = 0; G < F.length; G++) {
        C.addClass(
          ...
          'yui-toolbar-' +
            ...
            '-' +
            (F[G].value
              ? F[G].value.replace(/ /g, '-').toLowerCase()
              : F[G]._oText.nodeValue.replace(/ /g, '-').toLowerCase())
        );
        C.addClass(
          F[G].element,
          'yui-toolbar-' +
            I.get('value') +
            '-' +
            (F[G].value ? F[G].value.replace(/ /g, '-') : F[G]._oText.nodeValue.replace(/ /g, '-'))
        );
      }
    },
    ...
    _makeColorButton: function (E) {
      ...
      E.menu.beforeShowEvent.subscribe(
        function () {
...
(function () {
  ...
  YAHOO.widget.SimpleEditor = function (H, M) {
    ...
    var I = { element: null, attributes: G },
      F = null;
    if (D.isString(H)) {
      F = H;
    } else {
  ...
  YAHOO.extend(YAHOO.widget.SimpleEditor, YAHOO.util.Element, {
    ...
    _keyMap: {
      SELECT_ALL: { key: 65, mods: ['ctrl'] },
      CLOSE_WINDOW: { key: 87, mods: ['shift', 'ctrl'] },
      FOCUS_TOOLBAR: { key: 27, mods: ['shift'] },
      FOCUS_AFTER: { key: 27 },
      FONT_SIZE_UP: { key: 38, mods: ['shift', 'ctrl'] },
      FONT_SIZE_DOWN: { key: 40, mods: ['shift', 'ctrl'] },
      CREATE_LINK: { key: 76, mods: ['shift', 'ctrl'] },
      BOLD: { key: 66, mods: ['shift', 'ctrl'] },
      ITALIC: { key: 73, mods: ['shift', 'ctrl'] },
      UNDERLINE: { key: 85, mods: ['shift', 'ctrl'] },
      UNDO: { key: 90, mods: ['ctrl'] },
      REDO: { key: 90, mods: ['shift', 'ctrl'] },
      JUSTIFY_LEFT: { key: 219, mods: ['shift', 'ctrl'] },
      JUSTIFY_CENTER: { key: 220, mods: ['shift', 'ctrl'] },
      JUSTIFY_RIGHT: { key: 221, mods: ['shift', 'ctrl'] },
    },
    ...
    _setInitialContent: function (F) {
      ...
      if (
        ...
        this.get('iframe').get('element').src =
          'data:text/html;charset=utf-8,' + encodeURIComponent(G);
      }
    ...
    _fixNodes: function () {
      try {
        ...
        for (var I = 0; I < H.length; I++) {
          if (H[I].parentNode) {
            if (
              D.isObject(this.invalidHTML[H[I].tagName.toLowerCase()]) &&
              this.invalidHTML[H[I].tagName.toLowerCase()].keepContents
            ) {
    ...
    _listFix: function (K) {
      ...
      if (
        K.keyCode &&
        ((!this.browser.webkit3 && K.keyCode == 25) ||
          ((this.browser.webkit3 || !this.browser.webkit) && K.keyCode == 9 && K.shiftKey))
      ) {
      ...
      if (!this.get('disabled')) {
        if (this.STOP_NODE_CHANGE) {
          ...
          if (M) {
            ...
            M.set(
              'label',
              '<span class="yui-toolbar-fontname-' + this._cleanClassName(Y) + '">' + Y + '</span>'
            );
    ...
    initAttributes: function (E) {
      ...
      this.setAttributeConfig('height', {
        ...
        method: function (G) {
          if (this._rendered) {
            if (this.get('animate')) {
              var H = new YAHOO.util.Anim(
                this.get('iframe').get('parentNode'),
                { height: { to: parseInt(G, 10) } },
                0.5
              );
              H.animate();
            } else {
    ...
    _handleAfterNodeChange: function () {
      ...
      if (J) {
        ...
        var N =
          '<span class="yui-toolbar-fontname-' + this._cleanClassName(H) + '">' + H + '</span>';
        if (J.get('label') != N) {
          J.set('label', N);
      ...
      var E = function () {
        ...
        if (J !== '' && J !== null) {
          ...
          if (
            I.indexOf(':/' + '/') == -1 &&
            I.substring(0, 1) != '/' &&
            I.substring(0, 6).toLowerCase() != 'mailto'
          ) {
      ...
      if ((this.browser.webkit && !this.browser.webkit4) || this.browser.opera) {
        if (this._isElement(c, 'li') && this._isElement(c.parentNode, d)) {
          ...
          for (N = 0; N < this.currentElement.length; N++) {
            ...
            K.innerHTML =
              this.currentElement[N].innerHTML + '<span class="yui-non">&nbsp;</span>&nbsp;';
            Z.appendChild(K);
            if (N > 0) {
        ...
        if (
          (this._isElement(G, 'li') && this._isElement(G.parentNode, d)) ||
          (this.browser.ie && this._isElement(this._getRange().parentElement, 'li')) ||
          (this.browser.ie && this._isElement(G, 'ul')) ||
          (this.browser.ie && this._isElement(G, 'ol'))
        ) {
          if (this.browser.ie) {
            if (
              (this.browser.ie && this._isElement(G, 'ul')) ||
    ...
    _createCurrentElement: function (G, J) {
      ...
      if (!this._hasSelection()) {
        ...
        if (!this._isElement(this._getSelectedElement(), 'body')) {
          ...
          V[V.length] = this._getDoc().getElementsByTagName(
        ...
        for (var S = 0; S < E.length; S++) {
          if (
            YAHOO.util.Dom.getStyle(E[S], 'font-family') == 'yui-tmp' ||
            (E[S].face && E[S].face == 'yui-tmp')
          ) {
            if (G !== 'span') {
              F = M(G, J);
            } else {
    ...
    destroy: function () {
      ...
      this.get('element_cont')
        .get('parentNode')
        .replaceChild(E, this.get('element_cont').get('element'));
  ...
  YAHOO.widget.EditorInfo = {
    ...
    saveAll: function (G) {
      var F,
        H,
        E = YAHOO.widget.EditorInfo._instances;
      if (G) {
        for (F in E) {
...
(function () {
  ...
  YAHOO.extend(YAHOO.widget.Editor, YAHOO.widget.SimpleEditor, {
    ...
    _handleCreateLinkWindowClose: function () {
      ...
      if (G && G.value) {
        ...
        if (
          J.indexOf(':/' + '/') == -1 &&
          J.substring(0, 1) != '/' &&
          J.substring(0, 6).toLowerCase() != 'mailto'
        ) {
    ...
    _handleInsertImageClick: function () {
      ...
      this.on('afterExecCommand', function () {
        var H = this.currentElement[0],
          ...
          J = 0,
          S = false,
          I = new YAHOO.widget.EditorWindow('insertimage', { width: '415px' });
        if (!H) {
          H = this._getSelectedElement();
        }
        if (H) {
      ...
      if (E && E.value && E.value != this.STR_IMAGE_HERE) {
        ...
        if (I.value) {
          ...
          if (
            K.indexOf(':/' + '/') == -1 &&
            K.substring(0, 1) != '/' &&
            K.substring(0, 6).toLowerCase() != 'mailto'
          ) {
    ...
    cmd_heading: function (I) {
      ...
      if (I == this.STR_NONE) {
        if (
          (J && J.tagName && J.tagName.toLowerCase().substring(0, 1) == 'h') ||
          (J &&
            J.parentNode &&
            J.parentNode.tagName &&
            J.parentNode.tagName.toLowerCase().substring(0, 1) == 'h')
        ) {
        ...
        if (
          this._isElement(E, 'h1') ||
          this._isElement(E, 'h2') ||
          this._isElement(E, 'h3') ||
          this._isElement(E, 'h4') ||
          this._isElement(E, 'h5') ||
          this._isElement(E, 'h6')
        ) {
          G = this._swapEl(E, I);
          this._selectNode(G);
          this.currentElement[0] = G;
