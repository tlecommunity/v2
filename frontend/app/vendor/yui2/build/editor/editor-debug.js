(function () {
  ...
  YAHOO.widget.ToolbarButton = function (el, attrs) {
    ...
    YAHOO.widget.ToolbarButton.superclass.constructor.call(
      this,
      oConfig.element,
      oConfig.attributes
    );
...
(function () {
  ...
  var getButton = function (id) {
    ...
    if (
      !(button instanceof YAHOO.widget.ToolbarButton) &&
      !(button instanceof YAHOO.widget.ToolbarButtonAdvanced)
    ) {
  ...
  YAHOO.widget.Toolbar = function (el, attrs) {
    ...
    if (!oConfig.element) {
      ...
      if (local_attrs.container && Dom.get(local_attrs.container)) {
        YAHOO.log(
          'Container found in config appending to it (' + Dom.get(local_attrs.container).id + ')',
          'info',
          'Toolbar'
        );
  ...
  YAHOO.extend(YAHOO.widget.Toolbar, YAHOO.util.Element, {
      ...
      for (var i = 0; i < items.length; i++) {
        Dom.addClass(
          ...
          'yui-toolbar-' +
            ...
            '-' +
            (items[i].value
              ? items[i].value.replace(/ /g, '-').toLowerCase()
              : items[i]._oText.nodeValue.replace(/ /g, '-').toLowerCase())
        );
        Dom.addClass(
          items[i].element,
          'yui-toolbar-' +
            ...
            '-' +
            (items[i].value
              ? items[i].value.replace(/ /g, '-')
              : items[i]._oText.nodeValue.replace(/ /g, '-'))
        );
      }
    },
    /**
          ...
          if (this._titlebar) {
            ...
            if (collapse) {
              ...
              Event.addListener(
                ...
                function () {
                  if (
                    Dom.hasClass(this.get('cont').parentNode, 'yui-toolbar-container-collapsed')
                  ) {
    ...
    addButton: function (oButton, after) {
      ...
      if (skip) {
        ...
        if (oButton.type == 'menu' || oButton.type == 'split' || oButton.type == 'select') {
          if (Lang.isArray(oButton.menu)) {
            YAHOO.log(
              'Button type is (' + oButton.type + '), doing extra renderer work.',
              'info',
              'Toolbar'
            );
    ...
    _makeColorButton: function (_oButton) {
      ...
      _oButton.menu.beforeShowEvent.subscribe(
        function () {
    ...
    _makeSpinButton: function (_button, oButton) {
      ...
      var _intKeyUp = function (ev) {
        if (ev.keyCode == 38) {
          ...
          _intDown(ev);
        } else if (ev.keyCode == 107 && ev.shiftKey) {
          //Plus Key
          _intUp(ev);
        } else if (ev.keyCode == 109 && ev.shiftKey) {
          //Minus Key
          _intDown(ev);
        }
      };
      ...
      if (doEvent) {
        ...
        if (info.type == 'select') {
          ...
          if (button.buttonType == 'rich') {
            ...
            button.set(
              'label',
              '<span class="yui-toolbar-' +
                info.menucmd +
                '-' +
                info.value.replace(/ /g, '-').toLowerCase() +
                '">' +
                txt +
                '</span>'
            );
     ...
     */
    destroy: function () {
      var len = this._configuredButtons.length,
        j,
        i,
        b;
      for (b = 0; b < len; b++) {
    ...
    toString: function () {
      return (
        'Toolbar (#' + this.get('element').id + ') with ' + this._buttonList.length + ' buttons.'
      );
    },
  });
  /**
...
(function () {
  ...
  YAHOO.widget.SimpleEditor = function (el, attrs) {
    ...
    YAHOO.widget.SimpleEditor.superclass.constructor.call(
      this,
      oConfig.element,
      oConfig.attributes
    );
  ...
  YAHOO.extend(YAHOO.widget.SimpleEditor, YAHOO.util.Element, {
     ...
     */
    _defaultCSS:
      'html { height: 95%; } body { padding: 7px; background-color: #fff; font: 13px/1.22 arial,helvetica,clean,sans-serif;*font-size:small;*font:x-small; } a, a:visited, a:hover { color: blue !important; text-decoration: underline !important; cursor: text !important; } .warning-localfile { border-bottom: 1px dashed red !important; } .yui-busy { cursor: wait !important; } img.selected { border: 2px dotted #808080; } img { cursor: pointer !important; border: none; } body.ptags.webkit div.yui-wk-p { margin: 11px 0; } body.ptags.webkit div.yui-wk-div { margin: 0; }',
    /**
    ...
    _toggleDesignMode: function () {
      YAHOO.log(
        'It is not recommended to use this method and it will be depreciated.',
        'warn',
        'SimpleEditor'
      );
    ...
    _setInitialContent: function (raw) {
      ...
      if (
        ...
        this.get('iframe').get('element').src =
      ...
      for (var i = 0; i < path.length; i++) {
        ...
        if (!Dom.hasClass(path[i], 'yui-non')) {
          if (Dom.hasClass(path[i], 'yui-tag')) {
            ...
            if (
              classPath.indexOf('yui') != -1 ||
              classPath.toLowerCase().indexOf('apple-style-span') != -1
            ) {
              classPath = '';
            }
              ...
              if (path[i].getAttribute('href', 2)) {
                pathStr +=
                  ...
                  path[i]
                    ...
                    .replace('mailto:', '')
                    .replace('http:/' + '/', '')
                    .replace('https:/' + '/', ''); //May need to add others here ftp
              }
              break;
            case 'img':
              var h = path[i].height;
          ...
          if (pathStr.length > 10) {
            pathStr =
              '<span title="' + pathStr + '">' + pathStr.substring(0, 10) + '...' + '</span>';
          } else {
    ...
    _fixNodes: function () {
      try {
        ...
        for (var h = 0; h < els.length; h++) {
          if (els[h].parentNode) {
            if (
              Lang.isObject(this.invalidHTML[els[h].tagName.toLowerCase()]) &&
              this.invalidHTML[els[h].tagName.toLowerCase()].keepContents
            ) {
    ...
    _handleClick: function (ev) {
      var ret = this.fireEvent('beforeEditorClick', {
    ...
    _handleMouseUp: function (ev) {
      var ret = this.fireEvent('beforeEditorMouseUp', {
    ...
    _handleMouseDown: function (ev) {
      var ret = this.fireEvent('beforeEditorMouseDown', {
    ...
    _handleDoubleClick: function (ev) {
      ...
      } else if (this._hasParent(sel, 'a')) {
        //Handle elements inside an a
    ...
    _handleKeyUp: function (ev) {
      var ret = this.fireEvent('beforeEditorKeyUp', {
    ...
    _handleKeyPress: function (ev) {
      var ret = this.fireEvent('beforeEditorKeyPress', {
      ...
      if (this.get('allowNoEdit')) {
        ...
        if (ev && ev.keyCode && ev.keyCode == 63272) {
          //Forward delete key
          YAHOO.log(
            'allowNoEdit is set, forward delete key has been disabled',
            'warn',
            'SimpleEditor'
          );
          Event.stopEvent(ev);
        ...
        if (!this.browser.webkit3) {
          if (ev.keyCode && ev.keyCode == 122 && ev.metaKey) {
            ...
            if (this._hasParent(this._getSelectedElement(), 'li')) {
              YAHOO.log(
                'We are in an LI and we found CMD + z, stopping the event',
                'warn',
                'SimpleEditor'
              );
              Event.stopEvent(ev);
    ...
    _handleKeyDown: function (ev) {
      var ret = this.fireEvent('beforeEditorKeyDown', {
            ...
            if (this._checkKey(this._keyMap.CREATE_LINK, ev)) {
              ...
              if (this.get('limitCommands')) {
                if (!this.toolbar.getButtonByValue('createlink')) {
                  YAHOO.log(
                    'Toolbar Button for (createlink) was not found, skipping exec.',
                    'info',
                    'SimpleEditor'
                  );
                  makeLink = false;
            ...
            if (this.browser.gecko) {
              ...
              if (!this._hasParent(tar, 'li')) {
                if (this._hasParent(tar, 'p')) {
                  ...
                  YAHOO.log(
                    'Something went wrong with paragraphs, please file a bug!!',
                    'error',
                    'SimpleEditor'
                  );
                  doExec = true;
    ...
    _listFix: function (ev) {
      ...
      if (
        ev.keyCode &&
        ((!this.browser.webkit3 && ev.keyCode == 25) ||
          ((this.browser.webkit3 || !this.browser.webkit) && ev.keyCode == 9 && ev.shiftKey))
      ) {
        ...
        if (this._hasParent(testLi, 'li')) {
          ...
          if (this.browser.webkit) {
            this._getSelection().setBaseAndExtent(
              ...
              1,
              par.firstChild,
              par.firstChild.innerText.length
            );
            if (!this.browser.webkit3) {
    ...
    _nodeChange: function (force) {
      ...
      var beforeNodeChange = this.fireEvent('beforeNodeChange', {
      ...
      if (!this.get('disabled')) {
        if (this.STOP_NODE_CHANGE) {
          ...
          if (fn_button) {
            ...
            fn_button.set(
              'label',
              '<span class="yui-toolbar-fontname-' +
                this._cleanClassName(family) +
                '">' +
                family +
                '</span>'
            );
    ...
    _handleToolbarClick: function (ev) {
      ...
      if (this.STOP_EXEC_COMMAND) {
        YAHOO.log(
          'execCommand skipped because we found the STOP_EXEC_COMMAND flag set to true',
          'warn',
          'SimpleEditor'
        );
    ...
    _setupAfterElement: function () {
      if (!this.beforeElement) {
        ...
        this.get('element_cont')
          .get('firstChild')
          .insertBefore(this.beforeElement, this.toolbar.get('nextSibling'));
      }
          ...
          if (this._rendered) {
            ...
            if (this.get('animate')) {
              var anim = new YAHOO.util.Anim(
                this.get('iframe').get('parentNode'),
                {
                  height: {
    ...
    _handleAfterNodeChange: function () {
      ...
      if (fn_button) {
        for (var b = 0; b < fn_button._configs.menu.value.length; b++) {
          if (
            family &&
            fn_button._configs.menu.value[b].text.toLowerCase() == family.toLowerCase()
          ) {
            validFont = true;
    ...
    _handleInsertImageClick: function () {
      if (this.get('limitCommands')) {
        if (!this.toolbar.getButtonByValue('insertimage')) {
          YAHOO.log(
            'Toolbar Button for (insertimage) was not found, skipping exec.',
            'info',
            'SimpleEditor'
          );
          return false;
    ...
    _handleCreateLinkClick: function () {
      ...
      var _handleAEC = function () {
        ...
        if (str !== '' && str !== null) {
          ...
          if (
            urlValue.indexOf(':/' + '/') == -1 &&
            urlValue.substring(0, 1) != '/' &&
            urlValue.substring(0, 6).toLowerCase() != 'mailto'
          ) {
    ...
    execCommand: function (action, value) {
      ...
      if (this.get('limitCommands')) {
        if (!this.toolbar.getButtonByValue(action)) {
          YAHOO.log(
            'Toolbar Button for (' + action + ') was not found, skipping exec.',
            'info',
            'SimpleEditor'
          );
          exec = false;
      ...
      if (typeof this['cmd_' + action.toLowerCase()] == 'function' && exec) {
        YAHOO.log(
          'Found execCommand override method: (cmd_' + action.toLowerCase() + ')',
          'info',
          'SimpleEditor'
        );
      ...
      if (exec) {
        ...
        YAHOO.log(
          'OVERRIDE::execCommand::(' + action + '),(' + value + ') skipped',
          'warn',
          'SimpleEditor'
        );
      }
      ...
      if ((this.browser.webkit && !this.browser.webkit4) || this.browser.opera) {
        if (this._isElement(selEl, 'li') && this._isElement(selEl.parentNode, tag)) {
          ...
          for (li = 0; li < this.currentElement.length; li++) {
            ...
            newli.innerHTML =
              this.currentElement[li].innerHTML + '<span class="yui-non">&nbsp;</span>&nbsp;';
          ...
          var b_tag = this.browser.opera ? '<BR>' : '<br>',
            items = list.firstChild.innerHTML.split(b_tag),
            i,
            item;
          if (items.length > 0) {
            list.innerHTML = '';
        ...
        if (
          (this._isElement(el, 'li') && this._isElement(el.parentNode, tag)) ||
          (this.browser.ie && this._isElement(this._getRange().parentElement, 'li')) ||
          (this.browser.ie && this._isElement(el, 'ul')) ||
          (this.browser.ie && this._isElement(el, 'ol'))
        ) {
          //we are in a list..
          YAHOO.log('We already have a list, undo it', 'info', 'SimpleEditor');
          if (this.browser.ie) {
    ...
    cmd_fontname: function (value) {
      ...
      if (
        ...
        YAHOO.log(
          'No selection and no selected element and we are in insert mode',
          'info',
          'SimpleEditor'
        );
    ...
    _createCurrentElement: function (tagName, tagStyle) {
      ...
      if (!this._hasSelection()) {
        ...
        for (var i = 0; i < _tmp.length; i++) {
          if (
            YAHOO.util.Dom.getStyle(_tmp[i], 'font-family') == 'yui-tmp' ||
    ...
    cleanHTML: function (html) {
      ...
      if (markup == 'semantic' || markup == 'xhtml' || markup == 'css') {
        html = html.replace(
          new RegExp('<font([^>]*)face="([^>]*)">(.*?)</font>', 'gi'),
          '<span $1 style="font-family: $2;">$3</span>'
        );
    ...
    filter_safari: function (html) {
      if (this.browser.webkit) {
        ...
        html = html.replace(
          /<span class="Apple-tab-span" style="white-space:pre">([^>])<\/span>/gi,
          '&nbsp;&nbsp;&nbsp;&nbsp;'
        );
    ...
    destroy: function () {
      ...
      this.get('element_cont')
        .get('parentNode')
        .replaceChild(textArea, this.get('element_cont').get('element'));
  ...
  YAHOO.extend(YAHOO.widget.Editor, YAHOO.widget.SimpleEditor, {
        ...
        this._defaultImageToolbarConfig = {
          ...
          buttons: [
            ...
            { type: 'separator' },
            {
              ...
              buttons: [{ type: 'spin', label: '0', value: 'padding', range: [0, 50] }],
            },
            { type: 'separator' },
            {
              group: 'border',
    ...
    initAttributes: function (attr) {
      ...
      this.setAttributeConfig('hiddencss', {
        value:
          attr.hiddencss ||
          '.yui-hidden font, .yui-hidden strong, .yui-hidden b, .yui-hidden em, .yui-hidden i, .yui-hidden u, .yui-hidden div,.yui-hidden p,.yui-hidden span,.yui-hidden img, .yui-hidden ul, .yui-hidden ol, .yui-hidden li, .yui-hidden table { border: 1px dotted #ccc; } .yui-hidden .yui-non { border: none; } .yui-hidden img { padding: 2px; }',
        writeOnce: true,
      });
    },
    /**
    ...
    _renderCreateLinkWindow: function () {
      var str =
        '<label for="' +
        this.get('id') +
        '_createlink_url"><strong>' +
        this.STR_LINK_URL +
        ':</strong> <input type="text" name="' +
        this.get('id') +
        '_createlink_url" id="' +
        this.get('id') +
        '_createlink_url" value=""></label>';
      str +=
        '<label for="' +
        this.get('id') +
        '_createlink_target"><strong>&nbsp;</strong><input type="checkbox" name="' +
        this.get('id') +
        '_createlink_target" id="' +
        this.get('id') +
        '_createlink_target" value="_blank" class="createlink_target"> ' +
        this.STR_LINK_NEW_WINDOW +
        '</label>';
      str +=
        '<label for="' +
        this.get('id') +
        '_createlink_title"><strong>' +
        this.STR_LINK_TITLE +
        ':</strong> <input type="text" name="' +
        this.get('id') +
        '_createlink_title" id="' +
        this.get('id') +
        '_createlink_title" value=""></label>';
    ...
    _renderInsertImageWindow: function () {
      ...
      var str =
        '<label for="' +
        this.get('id') +
        '_insertimage_url"><strong>' +
        this.STR_IMAGE_URL +
        ':</strong> <input type="text" id="' +
        this.get('id') +
        '_insertimage_url" value="" size="40"></label>';
      var body = document.createElement('div');
      body.innerHTML = str;
      ...
      var str2 =
        '<label for="' +
        this.get('id') +
        '_insertimage_title"><strong>' +
        this.STR_IMAGE_TITLE +
        ':</strong> <input type="text" id="' +
        this.get('id') +
        '_insertimage_title" value="" size="40"></label>';
      str2 +=
        '<label for="' +
        this.get('id') +
        '_insertimage_link"><strong>' +
        this.STR_LINK_URL +
        ':</strong> <input type="text" name="' +
        this.get('id') +
        '_insertimage_link" id="' +
        this.get('id') +
        '_insertimage_link" value=""></label>';
      str2 +=
        '<label for="' +
        this.get('id') +
        '_insertimage_target"><strong>&nbsp;</strong><input type="checkbox" name="' +
        this.get('id') +
        '_insertimage_target_" id="' +
        this.get('id') +
        '_insertimage_target" value="_blank" class="insertimage_target"> ' +
        this.STR_LINK_NEW_WINDOW +
        '</label>';
      var div = document.createElement('div');
      div.innerHTML = str2;
      ...
      hw.innerHTML +=
        '<span tabIndex="-1"><input type="text" size="3" value="" id="' +
        this.get('id') +
        '_insertimage_width"> x <input type="text" size="3" value="" id="' +
        this.get('id') +
        '_insertimage_height"></span>';
      cont.insertBefore(hw, cont.firstChild);

      Event.onAvailable(
        this.get('id') + '_insertimage_width',
        function () {
          Event.on(
            this.get('id') + '_insertimage_width',
            'blur',
      ...
      Event.onAvailable(
        this.get('id') + '_insertimage_height',
        function () {
          Event.on(
            this.get('id') + '_insertimage_height',
            'blur',
        ...
        Event.on(
          ...
          function () {
            ...
            if (this._isLocalFile(url.value)) {
              ...
              //Adobe AIR Code
              if (
                (this.browser.webkit && !this.browser.webkit3) ||
                this.browser.air ||
                this.browser.opera
              ) {
    ...
    _handleInsertImageClick: function () {
      ...
      this.on('afterExecCommand', function () {
        ...
        bs_button.set(
          'label',
          '<span class="yui-toolbar-bordersize-' + bsize + '">' + bSizeStr + '</span>'
        );
    ...
    _renderPanel: function () {
      ...
      Dom.addClass(
        body,
        YAHOO.widget.Button && this._defaultToolbar.buttonType == 'advanced'
          ? 'good-button'
          : 'no-button'
      );
    ...
    moveWindow: function (force) {
      ...
      if (
        newXY[0] + wWidth * 2 >
        elXY[0] + parseInt(this.get('iframe').get('element').clientWidth, 10)
      ) {
        newXY[0] =
          elXY[0] + parseInt(this.get('iframe').get('element').clientWidth, 10) - wWidth * 2 - 5;
        align = 'right';
      }
      ...
      if (xDiff > 10 || yDiff > 10 || force) {
        //Only move the window if it's supposed to move more than 10px or force was passed (new window)
        var _knobLeft = 0,
          elW = 0;
        ...
        if (force) {
          ...
          if (this.get('animate')) {
            ...
            var iframeAnim = new YAHOO.util.Anim(
              panel.iframe,
              anim.attributes,
              0.5,
            ...
            var _knobAnim = new YAHOO.util.Anim(
              _knob,
              {
                left: {
                  to: _knobLeft,
    ...
    cmd_heading: function (value) {
      ...
      if (value == this.STR_NONE) {
        if (
          (_sel && _sel.tagName && _sel.tagName.toLowerCase().substring(0, 1) == 'h') ||
          (_sel &&
            _sel.parentNode &&
            _sel.parentNode.tagName &&
            _sel.parentNode.tagName.toLowerCase().substring(0, 1) == 'h')
        ) {
        ...
        if (
          this._isElement(_selEl, 'h1') ||
          this._isElement(_selEl, 'h2') ||
          this._isElement(_selEl, 'h3') ||
          this._isElement(_selEl, 'h4') ||
          this._isElement(_selEl, 'h5') ||
          this._isElement(_selEl, 'h6')
        ) {
