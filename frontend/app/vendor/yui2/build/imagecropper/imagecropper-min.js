(function () {
  ...
  YAHOO.extend(B, YAHOO.util.Element, {
    ...
    _handleKeyPress: function (G) {
      var E = A.getCharCode(G),
        F = false,
        H = G.shiftKey ? this.get('shiftKeyTick') : this.get('keyTick');
      switch (E) {
        case 37:
          this._moveEl('left', H);
          F = true;
    ...
    _handleBeforeResizeEvent: function (F) {
      var I = C.getRegion(this.get('element')),
        J = this._resize._cache,
    ...
    _syncBackgroundPosition: function () {
      ...
      this._setBackgroundPosition(
        -parseInt(this._resizeEl.style.left, 10),
        -parseInt(this._resizeEl.style.top, 10)
      );
    },
    ...
    _handleStartResizeEvent: function () {
      this._setConstraints(true);
      var I = this._resize._cache.height,
        F = this._resize._cache.width,
        H = parseInt(this._resize.getWrapEl().style.top, 10),
        E = parseInt(this._resize.getWrapEl().style.left, 10),
        G = 0,
        J = 0;
    ...
    _setConstraints: function (J) {
      ...
      var N = parseInt(H.get('height'), 10),
        F = parseInt(H.get('width'), 10);
      if (J) {
        N = H._cache.height;
        F = H._cache.width;
      }
    ...
    initAttributes: function (E) {
      ...
      this.setAttributeConfig('autoRatio', {
        validator: YAHOO.lang.isBoolean,
        value: E.autoRatio === false ? false : true,
        method: function (F) {
          if (this._resize) {
      ...
      this.setAttributeConfig('initHeight', {
        writeOnce: true,
        validator: YAHOO.lang.isNumber,
        value: E.initHeight || this.get('element').height / 4,
      });
      this.setAttributeConfig('initWidth', {
        validator: YAHOO.lang.isNumber,
        writeOnce: true,
        value: E.initWidth || this.get('element').width / 4,
      });
    },
    destroy: function () {
...
YAHOO.register('imagecropper', YAHOO.widget.ImageCropper, {
  version: '@VERSION@',
  build: '@BUILD@',
});
