  ...
  Paginator.prototype = {
      ...
      this.setAttributeConfig('containers', {
        ...
        validator: function (val) {
          ...
          for (var i = 0, len = val.length; i < len; ++i) {
            if (isString(val[i]) || (isObject(val[i]) && val[i].nodeType === 1)) {
              continue;
            }
            return false;
          }
      ...
      this.setAttributeConfig('recordOffset', {
        ...
        validator: function (val) {
          ...
          if (Paginator.isNumeric(val)) {
            val = +val;
            return total === UNLIMITED || total > val || (total === 0 && val === 0);
          }
    ...
    updateVisibility: function (e) {
      ...
      if (!e || e.type === 'alwaysVisibleChange' || !alwaysVisible) {
        ...
        if (totalRecords !== Paginator.VALUE_UNLIMITED && totalRecords <= rpp) {
          visible = false;
        }
        ...
        for (i = 0, len = this._containers.length; i < len; ++i) {
          Dom.setStyle(this._containers[i], 'display', visible ? '' : 'none');
        }
      }
    },
    ...
    setRowsPerPage: function (rpp, silent) {
      if (Paginator.isNumeric(rpp) && +rpp > 0 && +rpp !== this.get('rowsPerPage')) {
        if (this.get('updateOnChange') || silent) {
          this.set('rowsPerPage', rpp);
        } else {
          this.fireEvent('changeRequest', this.getState({ rowsPerPage: +rpp }));
        }
      }
    },
    ...
    setTotalRecords: function (total, silent) {
      if (Paginator.isNumeric(total) && +total >= 0 && +total !== this.get('totalRecords')) {
        if (this.get('updateOnChange') || silent) {
          this.set('totalRecords', total);
        } else {
          this.fireEvent('changeRequest', this.getState({ totalRecords: +total }));
        }
      }
    },
    ...
    setStartIndex: function (offset, silent) {
      if (Paginator.isNumeric(offset) && +offset >= 0 && +offset !== this.get('recordOffset')) {
        if (this.get('updateOnChange') || silent) {
          this.set('recordOffset', offset);
        } else {
          this.fireEvent('changeRequest', this.getState({ recordOffset: +offset }));
        }
      }
    },
    ...
    getState: function (changes) {
      ...
      currentState.page = ceil(currentState.recordOffset / currentState.rowsPerPage) + 1;
      ...
      if (state.totalRecords === 0) {
        state.recordOffset = state.page = 0;
      } else {
        ...
        state.recordOffset = normalizeOffset(offset, state.totalRecords, state.rowsPerPage);

        state.page = ceil(state.recordOffset / state.rowsPerPage) + 1;
      }

      state.records = [state.recordOffset, state.recordOffset + state.rowsPerPage - 1];

      // limit upper index to totalRecords - 1
      if (
        state.totalRecords !== UNLIMITED &&
    ...
    setState: function (state) {
      if (isObject(state)) {
        ...
        if (state.page && state.recordOffset === undefined) {
          state.recordOffset = (state.page - 1) * (state.rowsPerPage || this.get('rowsPerPage'));
        }
...
(function () {
  ...
  Paginator.ui.PageLinks.calculateRange = function (currentPage, totalPages, numPages) {
    ...
    if (totalPages !== UNLIMITED) {
      numPages = numPages === UNLIMITED ? totalPages : Math.min(numPages, totalPages);
    }
  ...
  Paginator.ui.PageLinks.prototype = {
    ...
    onClick: function (e) {
      ...
      if (t && YAHOO.util.Dom.hasClass(t, this.paginator.get('pageLinkClass'))) {
