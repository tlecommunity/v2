...
(function () {
    ...
    parseFlashVersion: function (ver) {
      ...
      if (YAHOO.lang.isString(ver)) {
        ...
        if (verSplit.length > 2) {
          ...
          flashVersion += parseInt(verSplit[2]) * 0.001;
        } else {
  ...
  YAHOO.widget.SWF = function (
    ...
    var shouldExpressInstall =
      canExpressInstall && !isFlashVersionRight && p_oAttributes['useExpressInstall'];
    ...
    if (oElement && (isFlashVersionRight || shouldExpressInstall) && flashURL) {
      ...
      for (var attribute in p_oAttributes.fixedAttributes) {
        if (possibleAttributes.hasOwnProperty(attribute.toLowerCase())) {
          objstring +=
            '<param name="' +
            YAHOO.lang.escapeHTML(attribute.toLowerCase()) +
            '" value="' +
            YAHOO.lang.escapeHTML(p_oAttributes.fixedAttributes[attribute]) +
            '"/>';
        }
      }
      ...
      for (var flashvar in p_oAttributes.flashVars) {
        ...
        if (Lang.isString(fvar)) {
          flashvarstring +=
            '&' +
            YAHOO.lang.escapeHTML(flashvar) +
            '=' +
            YAHOO.lang.escapeHTML(encodeURIComponent(fvar));
        }
      }
    ...
    _eventHandler: function (event) {
      if (event.type == 'swfReady') {
        ...
        YAHOO.log(
          event.message,
          event.category,
          this._host ? this._host.toString() : this.toString()
        );
      } else {
