...
YAHOO.deconcept.SWFObject = function (
  swf,
  id,
  w,
  h,
  ...
  getVariablePairs: function () {
    ...
    for (key in variables) {
      if (variables.hasOwnProperty(key)) {
        variablePairs[variablePairs.length] =
          YAHOO.lang.escapeHTML(key || '') +
          '=' +
          YAHOO.lang.escapeHTML(encodeURIComponent(variables[key] || ''));
      }
    }
  ...
  getSWFHTML: function () {
    ...
    if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
      ...
      swfNode =
        ...
        YAHOO.lang.escapeHTML(this.getAttribute('swf') || '') +
        '" width="' +
        YAHOO.lang.escapeHTML(this.getAttribute('width') || '') +
        '" height="' +
        YAHOO.lang.escapeHTML(this.getAttribute('height') || '') +
        '" style="' +
        YAHOO.lang.escapeHTML(this.getAttribute('style') || '') +
        '"';
      swfNode +=
        ' id="' +
        YAHOO.lang.escapeHTML(this.getAttribute('id') || '') +
        '" name="' +
        YAHOO.lang.escapeHTML(this.getAttribute('id') || '') +
        '" ';
      ...
      for (key in params) {
        if (params.hasOwnProperty(key)) {
          swfNode +=
            YAHOO.lang.escapeHTML(key || '') +
            '="' +
            YAHOO.lang.escapeHTML(params[key] || '') +
            '" ';
        }
      }
...
YAHOO.deconcept.SWFObjectUtil.getPlayerVersion = function () {
  ...
  if (navigator.plugins && navigator.mimeTypes.length) {
    ...
    if (x && x.description) {
      PlayerVersion = new YAHOO.deconcept.PlayerVersion(
        x.description
          .replace(/([a-zA-Z]|\s)+/, '')
          .replace(/(\s+r|\s+b[0-9]+)/, '.')
          .split('.')
...
YAHOO.extend(YAHOO.widget.FlashAdapter, YAHOO.util.AttributeProvider, {
  ...
  _embedSWF: function (
    ...
    var swfObj = new YAHOO.deconcept.SWFObject(
      swfURL,
      swfID,
      '100%',
      '100%',
