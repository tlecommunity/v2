  ...
  getVariablePairs: function () {
    ...
    for (b in c) {
      if (c.hasOwnProperty(b)) {
        a[a.length] =
          YAHOO.lang.escapeHTML(b || '') +
          '=' +
          YAHOO.lang.escapeHTML(encodeURIComponent(c[b] || ''));
      }
    }
    return a;
  },
    ...
    if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
      ...
      for (a in c) {
        if (c.hasOwnProperty(a)) {
          d +=
            '<param name="' +
            YAHOO.lang.escapeHTML(a || '') +
            '" value="' +
            YAHOO.lang.escapeHTML(c[a] || '') +
            '" />';
        }
      }
      b = this.getVariablePairs().join('&');
...
YAHOO.deconcept.SWFObjectUtil.getPlayerVersion = function () {
  ...
  if (navigator.plugins && navigator.mimeTypes.length) {
    ...
    if (a && a.description) {
      c = new YAHOO.deconcept.PlayerVersion(
        a.description
          .replace(/([a-zA-Z]|\s)+/, '')
          .replace(/(\s+r|\s+b[0-9]+)/, '.')
          .split('.')
