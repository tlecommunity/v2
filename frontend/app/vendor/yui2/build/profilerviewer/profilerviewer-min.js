(function () {
  ...
  YAHOO.lang.augmentObject(
    YAHOO.widget.ProfilerViewer,
    {
      ...
      STRINGS: {
        ...
        installFlashMessage:
          "Unable to load Flash content. The YUI Charts Control requires Flash Player 9.0.45 or higher. You can download the latest version of Flash Player from the <a href='http://www.adobe.com/go/getflashplayer'>Adobe Flash Player Download Center</a>.",
      },
      timeAxisLabelFunction: function (H) {
  ...
  D._initDataTable = function () {
    ...
    this._dataTable = new YAHOO.widget.DataTable(this._tableEl, L, H, {
      scrollable: true,
      height: this.get('tableHeight'),
      initialRequest: null,
      sortedBy: { key: 'total', dir: YAHOO.widget.DataTable.CLASS_DESC },
    });
  ...
  D._getSeriesDef = function (K) {
    ...
    for (var I = 0, H = J.group.length; I < H; I++) {
      ...
      G.push({
        ...
        xField: L.xField,
        style: { color: L.style.color, size: L.style.size },
      });
    }
    return G;
  };
  D._initChart = function () {
    ...
    var G = this;
    var H = new YAHOO.util.DataSource(
      function () {
    ...
    this._chartAxisDefinitionTime.labelFunction =
      'YAHOO.widget.ProfilerViewer.timeAxisLabelFunction';
    this._chartAxisDefinitionPercent = new YAHOO.widget.NumericAxis();
    this._chartAxisDefinitionPercent.labelFunction =
      'YAHOO.widget.ProfilerViewer.percentAxisLabelFunction';
    this._chartAxisDefinitionPlain = new YAHOO.widget.NumericAxis();
    this._chart = new YAHOO.widget.BarChart(this._chartEl, H, {
      yField: 'fn',
  ...
  D.initAttributes = function (G) {
    ...
    this.setAttributeConfig('sortedBy', {
      value: G.sortedBy || { key: 'total', dir: 'yui-dt-desc' },
    });
    ...
    this.setAttributeConfig('swfUrl', {
      value: G.swfUrl || 'http://yui.yahooapis.com/2.5.0/build/charts/assets/charts.swf',
    });
    this.setAttributeConfig('maxChartFunctions', {
      value: G.maxChartFunctions || 6,
      method: function (H) {
        if (this._rendered) {
          this._sizeChartCanvas();
        }
    ...
    this.setAttributeConfig('chartStyle', {
      value: G.chartStyle || {
        font: { name: 'Arial', color: 15658588, size: 12 },
    ...
    this.setAttributeConfig('chartSeriesDefinitions', {
      value: G.chartSeriesDefinitions || {
        total: {
          displayName: E.STRINGS.colHeads.total[0],
          xField: 'total',
          style: { color: '4d95dd', size: 20 },
          group: ['total'],
        },
        calls: {
          displayName: E.STRINGS.colHeads.calls[0],
          xField: 'calls',
          style: { color: 'edff9f', size: 20 },
          group: ['calls'],
        },
        avg: {
          displayName: E.STRINGS.colHeads.avg[0],
          xField: 'avg',
          style: { color: '209daf', size: 9 },
          group: ['avg', 'min', 'max'],
        },
        min: {
          displayName: E.STRINGS.colHeads.min[0],
          xField: 'min',
          style: { color: 'b6ecf4', size: 9 },
          group: ['avg', 'min', 'max'],
        },
        max: {
          displayName: E.STRINGS.colHeads.max[0],
          xField: 'max',
          style: { color: '29c7de', size: 9 },
          group: ['avg', 'min', 'max'],
        },
        pct: {
          displayName: E.STRINGS.colHeads.pct[0],
          xField: 'pct',
...
YAHOO.register('profilerviewer', YAHOO.widget.ProfilerViewer, {
  version: '@VERSION@',
  build: '@BUILD@',
});
