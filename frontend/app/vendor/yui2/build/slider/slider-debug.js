...
(function () {
    ...
    Slider,
    {
      ...
      getHorizSlider: function (sBGElId, sHandleElId, iLeft, iRight, iTickSize) {
        return new Slider(
          sBGElId,
          sBGElId,
          new YAHOO.widget.SliderThumb(sHandleElId, sBGElId, iLeft, iRight, 0, 0, iTickSize),
          'horiz'
        );
      },
      ...
      getVertSlider: function (sBGElId, sHandleElId, iUp, iDown, iTickSize) {
        return new Slider(
          sBGElId,
          sBGElId,
          new YAHOO.widget.SliderThumb(sHandleElId, sBGElId, 0, 0, iUp, iDown, iTickSize),
          'vert'
        );
      },
      ...
      getSliderRegion: function (sBGElId, sHandleElId, iLeft, iRight, iUp, iDown, iTickSize) {
        return new Slider(
          sBGElId,
          sBGElId,
          new YAHOO.widget.SliderThumb(sHandleElId, sBGElId, iLeft, iRight, iUp, iDown, iTickSize),
          'region'
        );
      },
  ...
  YAHOO.extend(Slider, YAHOO.util.DragDrop, {
      ...
      if (this.enableKeys) {
        ...
        if (changeValue) {
          if (t._isRegion) {
            ...
            this._setValue(Slider.SOURCE_KEY_EVENT, t._isHoriz ? h : v, true);
          }
          Event.stopEvent(e);
        }
      }
    ...
    _setRegionValue: function (source, newOffset, newOffset2, skipAnim, force, silent) {
      ...
      if (!t._isRegion) {
        this.logger.log(
          'Call to setRegionValue for non-region Slider ignored. Use setValue',
          'warn'
        );
        return false;
      }
    ...
    moveOneTick: function (finalCoord) {
      ...
      if (t._isRegion) {
        ...
        nextCoord = tmpX !== this.curCoord[0] || tmpY !== this.curCoord[1] ? [tmpX, tmpY] : null;
      } else if (t._isHoriz) {
        nextCoord = this._getNextX(this.curCoord, finalCoord);
      } else {
        nextCoord = this._getNextY(this.curCoord, finalCoord);
      ...
      if (nextCoord) {
        ...
        this.thumb.alignElWithMouse(
          t.getEl(),
          nextCoord[0] + this.thumbCenterPoint.x,
          nextCoord[1] + this.thumbCenterPoint.y
        );
...
YAHOO.extend(YAHOO.widget.SliderThumb, YAHOO.util.DD, {
  ...
  getXValue: function () {
    ...
    if (YAHOO.lang.isNumber(newOffset[0])) {
      ...
      this.logger.log("can't get offset, using old value: " + this.lastOffset[0]);
  ...
  getYValue: function () {
    ...
    if (YAHOO.lang.isNumber(newOffset[1])) {
      ...
      this.logger.log("can't get offset, using old value: " + this.lastOffset[1]);
...
(function () {
  ...
  DualSlider.prototype = {
    ...
    updateValue: function (silent) {
      ...
      if (min != this.minVal || max != this.maxVal) {
        ...
        thumbInnerWidth =
          this.minSlider.thumbCenterPoint[dim] + this.maxSlider.thumbCenterPoint[dim];
    ...
    _handleMouseUp: function (e) {
      YW.Slider.prototype.onMouseUp.apply(this.activeSlider, arguments);
    },
   ...
   */
  YW.Slider.getHorizDualSlider = function (bg, minthumb, maxthumb, range, iTickSize, initVals) {
   ...
   */
  YW.Slider.getVertDualSlider = function (bg, minthumb, maxthumb, range, iTickSize, initVals) {
