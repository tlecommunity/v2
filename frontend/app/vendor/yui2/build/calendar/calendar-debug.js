  ...
  Calendar.prototype = {
    ...
    setupConfig: function () {
      ...
      cfg.addProperty(DEF_CFG.MONTHS_LONG.key, {
        value: DEF_CFG.MONTHS_LONG.value,
        handler: this.configLocale,
      });
    ...
    _buildMonthLabel: function (date) {
      var monthLabel =
          this.Locale.LOCALE_MONTHS[date.getMonth()] + this.Locale.MY_LABEL_MONTH_SUFFIX,
    ...
    createCloseButton: function () {
      ...
      if (Calendar.IMG_ROOT !== null) {
        var img =
          Dom.getElementsByClassName(cssClose, 'img', lnk)[0] || document.createElement('img');
    ...
    removeCloseButton: function () {
      var btn =
        Dom.getElementsByClassName(this.Style.CSS_LINK_CLOSE, 'a', this.oDomContainer)[0] || null;
      if (btn) {
    ...
    renderHeader: function (html) {
      ...
      if (renderLeft) {
        ...
        var leftStyle =
          leftArrow === null ? '' : ' style="background-image:url(' + leftArrow + ')"';
        html[html.length] =
          '<a class="' +
      ...
      if (renderRight) {
        ...
        var rightStyle =
          rightArrow === null ? '' : ' style="background-image:url(' + rightArrow + ')"';
        html[html.length] =
          '<a class="' +
    ...
    renderBody: function (workingDate, html) {
      ...
      html[html.length] =
        '<tbody class="m' + (useDate.getMonth() + 1) + ' ' + this.Style.CSS_BODY + '">';
              ...
              for (var s = 0, l = rs.length; s < l; ++s) {
                ...
                switch (type) {
                  ...
                  case Calendar.RANGE:
                    ...
                    if (
                      workingDate.getTime() >= d1.getTime() &&
    ...
    render: function () {
      ...
      html[html.length] =
        ...
        ' y' +
        (workingDate.getFullYear() + this.Locale.YEAR_OFFSET) +
        '" id="' +
        this.id +
        '">';
    ...
    select: function (date) {
      ...
      if (validDates.length === 0) {
        this.logger.log(
          'All provided dates were OOB. beforeSelect and select events not fired',
          'info'
        );
      }
    ...
    deselect: function (date) {
      ...
      if (validDates.length === 0) {
        this.logger.log(
          'All provided dates were OOB. beforeDeselect and deselect events not fired'
        );
      }
    ...
    deselectCell: function (cellIndex) {
      ...
      if (!selectable) {
        this.logger.log(
          'The cell at cellIndex:' + cellIndex + ' is not a selectable/deselectable cell',
          'info'
        );
      }
      ...
      if (selectable) {
        ...
        if (cellDateIndex > -1) {
          if (
            (this.cfg.getProperty(DEF_CFG.PAGEDATE.key).getMonth() == dCellDate.getMonth() &&
              this.cfg.getProperty(DEF_CFG.PAGEDATE.key).getFullYear() ==
                dCellDate.getFullYear()) ||
            this.cfg.getProperty(DEF_CFG.OOM_SELECT.key)
          ) {
    ...
    isDateOOB: function (date) {
      ...
      return (
        (minDate && clearedDate.getTime() < minDate.getTime()) ||
    ...
    _parsePageDate: function (date) {
      ...
      if (date) {
        if (date instanceof Date) {
          ...
          month =
            parseInt(aMonthYear[this.cfg.getProperty(DEF_CFG.MY_MONTH_POSITION.key) - 1], 10) - 1;
          year =
            parseInt(aMonthYear[this.cfg.getProperty(DEF_CFG.MY_YEAR_POSITION.key) - 1], 10) -
    ...
    _parseDate: function (sDate) {
      ...
      if (aDate.length == 2) {
        ...
        rArray = [
          aDate[this.Locale.MDY_YEAR_POSITION - 1] - this.Locale.YEAR_OFFSET,
    ...
    _parseRange: function (startDate, endDate) {
      var dCurrent = DateMath.add(
          DateMath.getDate(startDate[0], startDate[1] - 1, startDate[2]),
          DateMath.DAY,
          1
        ),
...
YAHOO.widget.CalendarNavigator.prototype = {
  ...
  renderMonth: function (html) {
    ...
    var id = this.id + NAV.MONTH_SUFFIX,
      ...
      months = this.cal.cfg.getProperty(
        mf == YAHOO.widget.Calendar.SHORT ? 'MONTHS_SHORT' : 'MONTHS_LONG'
      ),
      h = html;
  ...
  _update: function (cal) {
    var date = YAHOO.widget.DateMath.getDate(
      this.getYear() - cal.cfg.getProperty('YEAR_OFFSET'),
      this.getMonth(),
      1
    );
  ...
  __getCfg: function (prop, bIsStr) {
    ...
    if (bIsStr) {
      return cfg !== true && cfg.strings && cfg.strings[prop]
        ? cfg.strings[prop]
        : DEF_CFG.strings[prop];
    } else {
