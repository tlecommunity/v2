  ...
  TV.prototype = {
    ...
    animateExpand: function (el, node) {
      ...
      if (this._expandAnim && this._animCount < this.maxAnim) {
        // this.locked = true;
        var tree = this;
        var a = Widget.TVAnim.getAnim(this._expandAnim, el, function () {
    ...
    animateCollapse: function (el, node) {
      ...
      if (this._collapseAnim && this._animCount < this.maxAnim) {
        // this.locked = true;
        var tree = this;
        var a = Widget.TVAnim.getAnim(this._collapseAnim, el, function () {
            ...
            switch (Lang.isString(type) && type.toLowerCase()) {
              ...
              default:
                ...
                if (Lang.isObject(NodeType)) {
                  for (
                    ThisType = NodeType;
                    ThisType && ThisType !== Widget.Node;
                    ThisType = ThisType.superclass.constructor
                  ) {}
                  if (ThisType) {
      ...
      var build = function (markup) {
        ...
        for (el = Dom.getFirstChild(markup); el; el = Dom.getNextSibling(el)) {
          switch (el.tagName.toUpperCase()) {
            ...
            case 'OL':
              this.logger.log(
                'ULs or OLs can only contain LI elements, not other UL or OL.  This will not work in some browsers',
                'error'
              );
              config = {
                type: 'text',
    ...
    _getEventTargetTdEl: function (ev) {
      ...
      while (
        target &&
        !(target.tagName.toUpperCase() == 'TD' && Dom.hasClass(target.parentNode, 'ygtvrow'))
      ) {
    ...
    _onClickEvent: function (ev) {
      ...
      if (
        Dom.hasClass(target, node.labelStyle) ||
        Dom.getAncestorByClassName(target, node.labelStyle)
      ) {
    ...
    _onMouseOverEvent: function (ev) {
      var target;
      if (
        (target = this._getEventTargetTdEl(ev)) &&
        (target = this.getNodeByElement(target)) &&
        (target = target.getToggleEl())
      ) {
  ...
  YAHOO.widget.Node.prototype = {
      ...
      if (Lang.isObject(oData)) {
        for (var property in oData) {
          if (oData.hasOwnProperty(property)) {
            if (
              property.charAt(0) != '_' &&
              !Lang.isUndefined(this[property]) &&
              !Lang.isFunction(this[property])
            ) {
    ...
    */
    ...
    getDepthStyle: function (depth) {
      return this.getAncestor(depth).nextSibling ? 'ygtvdepthcell' : 'ygtvblankdepthcell';
    },
    ...
    getNodeHtml: function () {
      ...
      for (var i = 0; i < this.depth; ++i) {
        sb[sb.length] =
          '<td class="ygtvcell ' + this.getDepthStyle(i) + '"><div class="ygtvspacer"></div></td>';
      }
    ...
    _removeFocus: function () {
      ...
      var el;
      while ((el = this._focusHighlightedItems.shift())) {
          ...
          if (!focused) {
            ...
            if (aEl.length) {
              ...
              Event.on(aEl, 'blur', function () {
                self.tree.fireEvent('focusChanged', {
...
(function () {
  ...
  YAHOO.extend(YAHOO.widget.DateNode, YAHOO.widget.TextNode, {
    ...
    fillEditorContainer: function (editorData) {
      var cal,
        container = editorData.inputContainer;
      ...
      if (editorData.nodeType != this._type) {
        ...
        editorData.inputObject = cal = new Calendar(
      ...
      cal.cfg.setProperty(
        'pagedate',
        pageDate[cal.cfg.getProperty('MDY_MONTH_POSITION') - 1] +
          delim +
          pageDate[cal.cfg.getProperty('MDY_YEAR_POSITION') - 1]
      );
      cal.cfg.fireQueue();
...
(function () {
  ...
  TNproto.fillEditorContainer = function (editorData) {
    ...
    if (editorData.nodeType != this._type) {
      ...
      editorData.inputElement = input = editorData.inputContainer.appendChild(
