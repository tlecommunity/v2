(function () {
  ...
  util.DataSourceBase = function (oLiveData, oConfigs) {
    if (oLiveData === null || oLiveData === undefined) {
      YAHOO.log(
        'Could not instantiate DataSource due to invalid live database',
        'error',
        this.toString()
      );
      return;
  ...
  lang.augmentObject(DS, {
    ...
    parseString: function (oData) {
      ...
      if (lang.isString(string)) {
        ...
        YAHOO.log(
          'Could not convert data ' + lang.dump(oData) + ' to type String',
          'warn',
          this.toString()
        );
        return null;
    ...
    parseNumber: function (oData) {
      ...
      if (lang.isNumber(number)) {
        ...
        YAHOO.log(
          'Could not convert data ' + lang.dump(oData) + ' to type Number',
          'warn',
          this.toString()
        );
        return null;
    ...
    convertNumber: function (oData) {
      YAHOO.log(
        'The method YAHOO.util.DataSourceBase.convertNumber() has been' +
          ' deprecated in favor of YAHOO.util.DataSourceBase.parseNumber()',
        'warn',
        this.toString()
      );
    ...
    parseDate: function (oData) {
      ...
      if (date instanceof Date) {
        ...
        YAHOO.log(
          'Could not convert data ' + lang.dump(oData) + ' to type Date',
          'warn',
          this.toString()
        );
        return null;
    ...
    convertDate: function (oData) {
      YAHOO.log(
        'The method YAHOO.util.DataSourceBase.convertDate() has been' +
          ' deprecated in favor of YAHOO.util.DataSourceBase.parseDate()',
        'warn',
        this.toString()
      );
  ...
  DS.prototype = {
      ...
      if (this.maxCacheEntries > 0) {
        ...
        // Look in local cache
          ...
          if (nCacheLength > 0) {
            ...
            for (var i = nCacheLength - 1; i >= 0; i--) {
              ...
              if (this.isCacheHit(oRequest, oCacheElem.request)) {
                ...
                if (i < nCacheLength - 1) {
                  ...
                  YAHOO.log(
                    'Refreshed cache position of the response for "' + oRequest + '"',
                    'info',
                    this.toString()
                  );
                }
            ...
            YAHOO.log(
              'The cached response for "' + lang.dump(oRequest) + '" is ' + lang.dump(oResponse),
              'info',
              this.toString()
            );
            return oResponse;
    ...
    setInterval: function (nMsec, oRequest, oCallback, oCaller) {
      if (lang.isNumber(nMsec) && nMsec >= 0) {
        YAHOO.log(
          'Enabling polling to live data for "' + oRequest + '" at interval ' + nMsec,
          'info',
          this.toString()
        );
        var oSelf = this;
        ...
        YAHOO.log(
          'Could not enable polling to live data for "' + oRequest + '" at interval ' + nMsec,
          'info',
          this.toString()
        );
      }
        ...
        if (ctype) {
          ...
          // xml
            ...
            (oRawResponse.nodeType === 9 ||
              oRawResponse.nodeType === 1 ||
              oRawResponse.nodeType === 11)
          ) {
            this.responseType = DS.TYPE_XML;
          } else if (
            oRawResponse &&
          ...
          } else if (YAHOO.lang.isObject(oRawResponse)) {
            // json
            this.responseType = DS.TYPE_JSON;
          } else if (YAHOO.lang.isString(oRawResponse)) {
            // text
            this.responseType = DS.TYPE_TEXT;
          }
        }
 ...
 */
    ...
    parseData: function (oRequest, oFullResponse) {
      if (lang.isValue(oFullResponse)) {
        ...
        YAHOO.log('Parsed generic data is ' + lang.dump(oParsedResponse), 'info', this.toString());
        return oParsedResponse;
      }
      YAHOO.log(
        'Generic data could not be parsed: ' + lang.dump(oFullResponse),
        'error',
        this.toString()
      );
      return null;
    ...
    parseArrayData: function (oRequest, oFullResponse) {
      if (lang.isArray(oFullResponse)) {
        ...
        YAHOO.log('Parsed array data is ' + lang.dump(oParsedResponse), 'info', this.toString());
        return oParsedResponse;
      }
      YAHOO.log(
        'Array data could not be parsed: ' + lang.dump(oFullResponse),
        'error',
        this.toString()
      );
      return null;
    ...
    parseTextData: function (oRequest, oFullResponse) {
      if (lang.isString(oFullResponse)) {
        if (
          ...
          YAHOO.log('Parsed text data is ' + lang.dump(oParsedResponse), 'info', this.toString());
          return oParsedResponse;
        }
      }
      YAHOO.log(
        'Text data could not be parsed: ' + lang.dump(oFullResponse),
        'error',
        this.toString()
      );
      return null;
      ...
      try {
        ...
        for (var m = schema.fields.length - 1; m >= 0; m--) {
          ...
          if (!field.parser && field.converter) {
            field.parser = field.converter;
            YAHOO.log(
              'The field property converter has been deprecated' + ' in favor of parser',
              'warn',
              this.toString()
            );
          }
          var parser =
            typeof field.parser === 'function' ? field.parser : DS.Parser[field.parser + ''];
          if (parser) {
            data = parser.call(this, data);
          }
          // Safety measure
    ...
    parseXMLData: function (oRequest, oFullResponse) {
      ...
      try {
        ...
        if (this.useXPath) {
          for (k in metaLocators) {
            oParsedResponse.meta[k] = YAHOO.util.DataSource._getLocationValue(
              metaLocators[k],
              oFullResponse
            );
          }
      ...
      if (bError) {
        YAHOO.log(
          'XML data could not be parsed: ' + lang.dump(oFullResponse),
          'error',
          this.toString()
        );
        oParsedResponse.error = true;
      } else {
        YAHOO.log('Parsed XML data is ' + lang.dump(oParsedResponse), 'info', this.toString());
      }
      return oParsedResponse;
    },
    ...
    parseJSONData: function (oRequest, oFullResponse) {
      ...
      if (lang.isObject(oFullResponse) && this.responseSchema.resultsList) {
        ...
        if (!bError) {
              ...
              if (r) {
                for (j = simpleFields.length - 1; j >= 0; --j) {
                  ...
                  rec[simpleFields[j].key] =
                    r[simpleFields[j].path] !== undefined ? r[simpleFields[j].path] : r[j];
                }
          ...
          YAHOO.log(
            'JSON data could not be parsed due to invalid responseSchema.resultsList or invalid response: ' +
              lang.dump(oFullResponse),
            'error',
            this.toString()
          );
        ...
        YAHOO.log(
          'JSON data could not be parsed: ' + lang.dump(oFullResponse),
          'error',
          this.toString()
        );
        oParsedResponse.error = true;
    ...
    parseHTMLTableData: function (oRequest, oFullResponse) {
      ...
      if (bError) {
        YAHOO.log(
          'HTML TABLE data could not be parsed: ' + lang.dump(oFullResponse),
          'error',
          this.toString()
        );
        oParsedResponse.error = true;
        ...
        YAHOO.log(
          'Parsed HTML TABLE data is ' + lang.dump(oParsedResponse),
          'info',
          this.toString()
        );
      }
  ...
  lang.extend(util.FunctionDataSource, DS, {
    ...
    makeConnection: function (oRequest, oCallback, oCaller) {
      ...
      var oRawResponse = this.scope
        ? this.liveData.call(this.scope, oRequest, this, oCallback)
        : this.liveData(oRequest, oCallback);
  ...
  lang.extend(util.ScriptNodeDataSource, DS, {
    ...
    generateRequestCallback: function (id) {
      return (
        '&' + this.scriptCallbackParam + '=YAHOO.util.ScriptNodeDataSource.callbacks[' + id + ']'
      );
    },
      ...
      util.ScriptNodeDataSource.callbacks[id] = function (oRawResponse) {
        if (
          ...
          if (oSelf.responseType === DS.TYPE_UNKNOWN) {
            ...
            } else if (oRawResponse.nodeName && oRawResponse.nodeName.toLowerCase() == 'table') {
              // table
          ...
          YAHOO.log(
            'DataSource ignored stale response for tId ' + tId + '(' + oRequest + ')',
            'info',
            oSelf.toString()
          );
        }
  ...
  lang.extend(util.XHRDataSource, DS, {
    ...
    makeConnection: function (oRequest, oCallback, oCaller) {
      ...
      var _xhrFailure = function (oResponse) {
        ...
        YAHOO.log(DS.ERROR_DATAINVALID + ': ' + oResponse.statusText, 'error', this.toString());
      ...
      if (this.connXhrMode == 'cancelStaleRequests') {
        ...
        if (oQueue.conn) {
          if (oConnMgr.abort) {
            ...
            YAHOO.log(
              'Could not find Connection Manager abort() function',
              'error',
              this.toString()
            );
          }
      ...
      if (oConnMgr && oConnMgr.asyncRequest) {
        ...
        YAHOO.log(
          'Could not find Connection Manager asyncRequest() function',
          'error',
          this.toString()
        );
  ...
  util.DataSource = function (oLiveData, oConfigs) {
    ...
    if (YAHOO.lang.isString(oLiveData)) {
      // strings default to xhr
      return new util.XHRDataSource(oLiveData, oConfigs);
    } else if (YAHOO.lang.isFunction(oLiveData)) {
      return new util.FunctionDataSource(oLiveData, oConfigs);
    } else {
      // ultimate default is local
      return new util.LocalDataSource(oLiveData, oConfigs);
    }
  };
...
YAHOO.util.Number = {
  ...
  format: function (n, cfg) {
    ...
    if (places < 0) {
      ...
      if (i > 0) {
        ...
        s =
          Number('.' + s)
            .toFixed(i)
            .slice(2) + new Array(s.length - i + 1).join('0');
      } else {
        s = '0';
      }
    } else {
    ...
    if (absN >= 1000) {
      ...
      bits[0] = bits[0].slice(0, i) + bits[0].slice(i).replace(/(\d{3})/g, sep + '$1');
    }
...
YAHOO.util.Number.format._applyFormat = function (tmpl, num, data) {
  return tmpl.replace(/\{(\w+)\}/g, function (_, token) {
    return token === 'number' ? num : token in data ? data[token] : '';
  });
};
  ...
  var Dt = {
    formats: {
      ...
      j: function (d) {
        ...
        var gmdate = new Date(
          '' + d.getFullYear() + '/' + (d.getMonth() + 1) + '/' + d.getDate() + ' GMT'
        );
        var ms = gmdate - gmd_1;
