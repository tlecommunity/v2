(function () {
  ...
  function l(x) {
    var w = this,
      v = 0,
      u = 0,
      t = w.get('isVertical') ? 'height' : 'width';
    v = P.call(w, t);
    u = v * x;
    return u;
  }
  ...
  YAHOO.extend(V, YAHOO.util.Element, {
    _rows: null,
    _cols: null,
    _animObj: null,
    _carouselEl: null,
    ...
    initAttributes: function (t) {
      ...
      u.setAttributeConfig('carouselItemEl', {
        validator: r.isString,
        value: t.carouselItemEl || 'LI',
      });
      u.setAttributeConfig('currentPage', { readOnly: true, value: 0 });
      u.setAttributeConfig('firstVisible', {
        method: u._setFirstVisible,
        validator: u._validateFirstVisible,
        value: t.firstVisible || u.CONFIG.FIRST_VISIBLE,
      });
      u.setAttributeConfig('selectOnScroll', {
        validator: r.isBoolean,
        value: t.selectOnScroll || true,
      });
      u.setAttributeConfig('numVisible', {
        setter: u._numVisibleSetter,
        method: u._setNumVisible,
        validator: u._validateNumVisible,
        value: t.numVisible || u.CONFIG.NUM_VISIBLE,
      });
      u.setAttributeConfig('numItems', {
        method: u._setNumItems,
        validator: u._validateNumItems,
        value: u._itemsTable.numItems,
      });
      u.setAttributeConfig('scrollIncrement', {
        validator: u._validateScrollIncrement,
        value: t.scrollIncrement || 1,
      });
      u.setAttributeConfig('selectedItem', {
        setter: u._selectedItemSetter,
        method: u._setSelectedItem,
        validator: r.isNumber,
        value: -1,
      });
      u.setAttributeConfig('revealAmount', {
        method: u._setRevealAmount,
        validator: u._validateRevealAmount,
        value: t.revealAmount || 0,
      });
      u.setAttributeConfig('isCircular', { validator: r.isBoolean, value: t.isCircular || false });
      u.setAttributeConfig('isVertical', {
        method: u._setOrientation,
        validator: r.isBoolean,
        value: t.isVertical || false,
      });
      u.setAttributeConfig('navigation', {
        method: u._setNavigation,
        validator: u._validateNavigation,
        value: t.navigation || { prev: null, next: null, page: null },
      });
      u.setAttributeConfig('animation', {
        validator: u._validateAnimation,
        value: t.animation || { speed: 0, effect: null },
      });
      u.setAttributeConfig('autoPlay', { validator: r.isNumber, value: t.autoPlay || 0 });
      u.setAttributeConfig('autoPlayInterval', {
        validator: r.isNumber,
        value: t.autoPlayInterval || 0,
      });
    ...
    getVisibleItems: function () {
      var v = this,
        t = v.get('firstVisible'),
        w = t + v.get('numVisible'),
        u = [];
      while (t < w) {
        u.push(v.getElementForItem(t));
        t++;
    ...
    replaceItem: function (AB, w) {
      ...
      if (r.isUndefined(w)) {
        ...
        v = u.id || g.generateId();
        AA._itemsTable.items.splice(w, 1, {
          item: x,
          className: AA.CLASSES.ITEM + (AB.className ? ' ' + AB.className : ''),
          id: v,
        });
    ...
    scrollPageBackward: function () {
      var v = this,
        w = v.get('isVertical'),
        u = v._cols,
        x = v.get('firstVisible'),
        t = x - v.get('numVisible');
      if (t < 0) {
        if (u) {
          t = x - u;
        }
    ...
    scrollPageForward: function () {
      var u = this,
        t = u._firstItem + u.get('numVisible');
      if (t > u.get('numItems')) {
        t = 0;
      }
    ...
    updatePagination: function () {
      ...
      var y = AB.get('numItems'),
        AC = AB.get('numVisible'),
        w = AB.get('firstVisible') + 1,
        x = AB.get('currentPage') + 1,
        t = AB.get('numPages'),
        v = {
          numVisible: AC,
          numPages: t,
          numItems: y,
          selectedItem: AB.get('selectedItem') + 1,
          currentPage: x,
          firstVisible: w,
          lastVisible: AB.get('lastVisible') + 1,
        },
        u = z.callback || {},
        AA = u.scope && u.obj ? u.obj : AB;
      z.el.innerHTML = r.isFunction(u.fn)
        ? u.fn.apply(AA, [z.template, v])
        : YAHOO.lang.substitute(z.template, v);
    },
    ...
    _itemClickHandler: function (x) {
      var AA = this,
        y = AA.get('carouselItemEl'),
        u = AA.get('element'),
        v,
        w,
        z = e.getTarget(x),
        t = z.tagName.toUpperCase();
    ...
    _loadItems: function (v) {
      var y = this,
        u = y.get('numItems'),
        w = y.get('numVisible'),
        x = y.get('revealAmount'),
        z = y._itemsTable.items.length,
        t = y.get('lastVisible');
      if (z > v && v + 1 >= w) {
    ...
    _getLastVisible: function () {
      var t = this;
      return t.get('currentPage') + 1 == t.get('numPages')
        ? t.get('numItems') - 1
        : t.get('firstVisible') + t.get('numVisible') - 1;
    },
    ...
    _syncUiForLazyLoading: function (x) {
      var AD = this,
        ...
        v = AD._itemsTable,
        z = v.items.length,
        AC = AD._findClosestSibling(x.last),
        AB = x.last,
        y = AB - AD.get('numVisible') + 1,
        t,
        u;
      for (var w = y; w <= AB; w++) {
        if (!v.loading[w] && !v.items[w]) {
          t = AD._createCarouselItem({
            className: AD.CLASSES.ITEM + ' ' + AD.CLASSES.ITEM_LOADING,
            content: AD.STRINGS.ITEM_LOADING_CONTENT,
            id: g.generateId(),
          });
          if (t) {
            if (AC) {
