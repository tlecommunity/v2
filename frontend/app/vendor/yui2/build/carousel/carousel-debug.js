...
(function () {
  ...
  function getStyle(el, style, type) {
    ...
    function getStyleIntVal(el, style) {
      ...
      if (
        style == 'marginRight' &&
        (YAHOO.env.ua.webkit || (YAHOO.env.ua.ie && YAHOO.env.ua.ie >= 9))
      ) {
    ...
    switch (style) {
      case 'height':
        ...
        if (value > 0) {
          value += getStyleIntVal(el, 'marginTop') + getStyleIntVal(el, 'marginBottom');
        } else {
          value =
            getStyleFloatVal(el, 'height') +
      ...
      case 'width':
        ...
        if (value > 0) {
          value += getStyleIntVal(el, 'marginLeft') + getStyleIntVal(el, 'marginRight');
        } else {
          value =
            getStyleFloatVal(el, 'width') +
  ...
  function getRevealSize() {
    ...
    sz = getCarouselItemSize.call(carousel, isVertical ? 'height' : 'width');
  ...
  function showNavigation(hide) {
    ...
    if (JS.isUndefined(hide)) {
      // show the navigation
      if (!JS.isUndefined(cfg.prev) && JS.isArray(cfg.prev) && !JS.isUndefined(cfg.prev[0])) {
        Dom.setStyle(cfg.prev[0], 'visibility', 'visible');
      }
      if (!JS.isUndefined(cfg.next) && JS.isArray(cfg.next) && !JS.isUndefined(cfg.next[0])) {
        Dom.setStyle(cfg.next[0], 'visibility', 'visible');
      }
      if (!JS.isUndefined(carousel._pages) && !JS.isUndefined(carousel._pages.el)) {
  ...
  function syncNavigation() {
    ...
    if (navigation.prev) {
      if (carousel.get('numItems') === 0 || carousel._firstItem === 0) {
        if (carousel.get('numItems') === 0 || !carousel.get('isCircular')) {
          Event.removeListener(navigation.prev, 'click', scrollPageBackward);
          Dom.addClass(navigation.prev, cssClass.FIRST_NAV_DISABLED);
          for (i = 0; i < carousel._navBtns.prev.length; i++) {
            carousel._navBtns.prev[i].setAttribute('disabled', 'true');
          }
      ...
      if (attach) {
        Event.on(navigation.prev, 'click', scrollPageBackward, carousel);
    ...
    if (navigation.next) {
      if (sentinel >= carousel.get('numItems')) {
        if (!carousel.get('isCircular')) {
          Event.removeListener(navigation.next, 'click', scrollPageForward);
          Dom.addClass(navigation.next, cssClass.DISABLED);
          for (i = 0; i < carousel._navBtns.next.length; i++) {
            carousel._navBtns.next[i].setAttribute('disabled', 'true');
          }
      ...
      if (attach) {
        Event.on(navigation.next, 'click', scrollPageForward, carousel);
    ...
    carousel.fireEvent(navigationStateChangeEvent, {
      next: carousel._nextEnabled,
      prev: carousel._prevEnabled,
    });
  }
  ...
  YAHOO.extend(Carousel, YAHOO.util.Element, {
    ...
    clearItems: function () {
      ...
      while (n > 0) {
        if (!carousel.removeItem(0)) {
          YAHOO.log('Item could not be removed - missing?', 'warn', WidgetName);
        }
        /*
    ...
    focus: function () {
      ...
      selected = selItem >= 0 ? carousel.getItem(selItem) : null;
      first = carousel.get('firstVisible');
      last = first + numVisible - 1;
      isSelectionInvisible = selItem < first || selItem > last;
      focusEl = selected && selected.id ? Dom.get(selected.id) : null;
      itemsTable = carousel._itemsTable;

      if (!selectOnScroll && isSelectionInvisible) {
        focusEl =
          itemsTable && itemsTable.items && itemsTable.items[first]
            ? Dom.get(itemsTable.items[first].id)
            : null;
      }
    ...
    init: function (el, attrs) {
      ...
      if (!el) {
        YAHOO.log(el + ' is neither an HTML element, nor a string', 'error', WidgetName);
        return;
      }
      ...
      carousel._itemsTable = { loading: {}, numItems: 0, items: [], size: 0 };
      ...
      if (el) {
        if (!el.id) {
          // in case the HTML element is passed
    ...
    initEvents: function () {
      ...
      carousel.on(renderEvent, function (ev) {
        if (carousel.get('selectedItem') === null || carousel.get('selectedItem') <= 0) {
          //in either case
      ...
      carousel.on('selectedItemChange', function (ev) {
        ...
        if (ev.newValue >= 0) {
          carousel._updateTabIndex(carousel.getElementForItem(ev.newValue));
        }
    ...
    removeItem: function (index) {
      ...
      if (item && item.length == 1) {
        ...
        carousel.fireEvent(itemRemovedEvent, { item: item[0], pos: index, ev: itemRemovedEvent });
        return true;
      }
    ...
    render: function (appendTo) {
      ...
      if (appendTo) {
        ...
        if (!Dom.inDocument(carousel.get('element'))) {
          YAHOO.log(
            'Nothing to render. The container should be ' +
      ...
      if (carousel.get('numItems') < 1) {
        YAHOO.log('No items in the Carousel to render', 'warn', WidgetName);
        return false;
      }
    ...
    scrollBackward: function () {
      var carousel = this;
      carousel.scrollTo(carousel._firstItem - carousel.get('scrollIncrement'));
    },
    ...
    scrollTo: function (item, dontSelect) {
      ...
      if (itemsTable.numItems === 0 || item == carousel._firstItem || carousel.isAnimating()) {
      ...
      if (animate) {
        carousel._animateAndSetCarouselOffset(offset, item, sentinel, dontSelect);
      } else {
    ...
    getPageForItem: function (item) {
      return Math.ceil((item + 1) / parseInt(this.get('numVisible'), 10));
    },
    ...
    selectPreviousItem: function () {
      ...
      if (selected == carousel._firstItem) {
        ...
        newpos = carousel.get('selectedItem') - carousel.get('scrollIncrement');
    ...
    updatePagination: function () {
      ...
      pagination.el.innerHTML = JS.isFunction(cb.fn)
        ? cb.fn.apply(scope, [pagination.template, replacements])
        : YAHOO.lang.substitute(pagination.template, replacements);
    },
    ...
    _animateAndSetCarouselOffset: function (offset, item, sentinel) {
      ...
      if (carousel.get('isVertical')) {
        animObj = new YAHOO.util.Motion(
          carousel._carouselEl,
          { top: { to: offset } },
          animCfg.speed,
          animCfg.effect
        );
    ...
    _getValidIndex: function (index) {
      ...
      if (index < 0) {
        index = isCircular ? Math.ceil(numItems / numVisible) * numVisible + index : 0;
    ...
    _itemClickHandler: function (ev) {
      ...
      while (target && target != container && target.id != carousel._carouselEl) {
        el = target.nodeName;
    ...
    _loadItems: function (last) {
      ...
      if (first > last && last + 1 >= numVisible) {
        ...
        first =
          last % numVisible || last == lastVisible
            ? last - (last % numVisible)
            : last - numVisible + 1;
      }
      ...
      for (child = parent.firstChild; child; child = child.nextSibling) {
        if (child.nodeType == 1) {
          ...
          if (node.toUpperCase() == domEl) {
            ...
            Dom.addClass(carousel._carouselEl, carousel.CLASSES.CAROUSEL_EL);
            YAHOO.log(
              'Found Carousel - ' + node + (child.id ? ' (#' + child.id + ')' : ''),
              WidgetName
            );
            found = true;
          }
      ...
      if (nav.length > 0) {
        for (i in nav) {
          if (nav.hasOwnProperty(i)) {
            el = nav[i];
            YAHOO.log(
              'Found Carousel previous page navigation - ' +
                el +
                (el.id ? ' (#' + el.id + ')' : ''),
              WidgetName
            );
            if (el.nodeName == 'INPUT' || el.nodeName == 'BUTTON' || el.nodeName == 'A') {
              // Anchor support in Nav (for SEO)
      ...
      if (nav.length > 0) {
        for (i in nav) {
          if (nav.hasOwnProperty(i)) {
            el = nav[i];
            YAHOO.log(
              'Found Carousel next page navigation - ' + el + (el.id ? ' (#' + el.id + ')' : ''),
              WidgetName
            );
            if (el.nodeName == 'INPUT' || el.nodeName == 'BUTTON' || el.nodeName == 'A') {
              // Anchor support in Nav (for SEO)
    ...
    _refreshUi: function () {
      ...
      sz = isVertical ? getStyle(item, 'width') : getStyle(item, 'height');

      Dom.setStyle(carousel._carouselEl, isVertical ? 'width' : 'height', sz + 'px');
    ...
    _setupCarouselNavigation: function () {
      ...
      navContainer = Dom.getElementsByClassName(
        cssClass.NAVIGATION,
        'DIV',
        carousel.get('element')
      );
      ...
      if (navContainer.length === 0) {
        navContainer = createElement('DIV', { className: cssClass.NAVIGATION });
        carousel.insertBefore(navContainer, Dom.getFirstChild(carousel.get('element')));
      } else {
      ...
      if (JS.isString(nav.prev) || JS.isArray(nav.prev)) {
        ...
        prevButton = createElement('SPAN', { className: cssClass.BUTTON + cssClass.FIRST_NAV });
        // XXX: for IE 6.x
      ...
      if (JS.isString(nav.next) || JS.isArray(nav.next)) {
        ...
        // TODO: separate method for creating a navigation button
        nextButton = createElement('SPAN', { className: cssClass.BUTTON + cssClass.NEXT_NAV });
        // XXX: for IE 6.x
    ...
    _setContainerSize: function (clip, attr) {
      ...
      if (isVertical) {
        size += getDimensions(carousel._carouselEl, 'height') + getStyle(carousel._navEl, 'height');
      } else {
    ...
    _setNumVisible: function (val) {
      // TODO: _setNumVisible should just be reserved for setting numVisible.
      var carousel = this;
    ...
    _setNumItems: function (val) {
      ...
      if (JS.isArray(carousel._itemsTable.items)) {
        if (carousel._itemsTable.items.length != num) {
          // out of sync
      ...
      if (!oel) {
        ...
        if (JS.isUndefined(obj.pos)) {
          ...
          if (!Dom.isAncestor(carouselEl, oel)) {
            if (!JS.isUndefined(itemsTable.items[obj.pos + 1])) {
              carouselEl.insertBefore(oel, Dom.get(itemsTable.items[obj.pos + 1].id));
            }
          }
        }
      }
    ...
    _updateNavButtons: function (el, setFocus) {
      ...
      if (el.nodeName.toUpperCase() == 'BUTTON' && Dom.hasClass(parent, cssClass.BUTTON)) {
        if (setFocus) {
          if (grandParent) {
    ...
    _updatePagerMenu: function () {
      ...
      if (!sel) {
        YAHOO.log('Unable to create the pager menu', 'error', WidgetName);
        return;
      }
