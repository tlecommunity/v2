  ...
  Y.Dom = {
    ...
    get: function (el) {
      ...
      if (el) {
        if (typeof el == 'string' || typeof el == 'number') {
          ...
          el = el.get('element');
        } else if (!el.nodeType && 'length' in el) {
          // array-like
          c = [];
    ...
    _hasClass: function (el, className) {
      ...
      if (el && className) {
        ...
        if (current) {
          // convert line breaks, tabs and other delims to spaces
    ...
    _replaceClass: function (el, classObj) {
      ...
      if (el && classObj) {
        ...
        if (!to) {
          ...
          current = Y.Dom._getAttribute(el, CLASS_NAME) || EMPTY;
          className = (
            SPACE + current.replace(Y.Dom._getClassRegex(from), SPACE + to).replace(/\s+/g, SPACE)
    ...
    getDocumentHeight: function () {
      var scrollHeight =
          document[COMPAT_MODE] != CSS1_COMPAT || isSafari
    ...
    _getAttribute: function (el, attr) {
      ...
      if (Y.Dom.DOT_ATTRIBUTES[attr]) {
        ...
        if (/^(?:href|src)$/.test(attr)) {
          // use IE flag to return exact value
    ...
    _toCamel: function (property) {
      ...
      return (
        c[property] ||
        (c[property] = property.indexOf('-') === -1 ? property : property.replace(/-([a-z])/gi, tU))
      );
    },
...
(function () {
    ...
    ComputedStyle = {
      ...
      get: function (el, property) {
        ...
        if (property === OPACITY) {
          ...
          // no need to convert
          value = current;
        } else if (Y.Dom.IE_COMPUTED[property]) {
      ...
      getOffset: function (el, prop) {
        ...
        if (current == AUTO) {
          ...
          if (actual === undefined) {
            // likely "right" or "bottom"
            value = 0;
          }

          value = actual;
          if (re_size.test(prop)) {
            // account for box model diff
  ...
  IEComputed.top =
    ...
    IEComputed[WIDTH] =
    IEComputed[HEIGHT] =
      ComputedStyle.getOffset;

  IEComputed.color = ComputedStyle.getColor;

  IEComputed[BORDER_TOP_WIDTH] =
    IEComputed[BORDER_RIGHT_WIDTH] =
    IEComputed[BORDER_BOTTOM_WIDTH] =
    IEComputed[BORDER_LEFT_WIDTH] =
      ComputedStyle.getBorderWidth;

  IEComputed.marginTop =
    IEComputed.marginRight =
    IEComputed.marginBottom =
    IEComputed.marginLeft =
      ComputedStyle.getMargin;

  IEComputed.visibility = ComputedStyle.getVisibility;
  IEComputed.borderColor =
    IEComputed.borderTopColor =
    IEComputed.borderRightColor =
    IEComputed.borderBottomColor =
    IEComputed.borderLeftColor =
  ...
  Y.Dom.Color = {
    ...
    toRGB: function (val) {
      ...
      if (Y.Dom.Color.re_hex.exec(val)) {
        val =
          'rgb(' +
          [PARSE_INT(RE.$1, 16), PARSE_INT(RE.$2, 16), PARSE_INT(RE.$3, 16)].join(', ') +
          ')';
      }
      return val;
    },
