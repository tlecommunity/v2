...
class SpyStatus {
  ...
  handleColony(colony, buildings) {
    return lacuna.body
      .findBuilding(buildings, 'Intelligence Ministry')
      .then((intelMin) => {
        if (!intelMin) {
      ...
      .then((result) => {
        let table = new Table({
          head: ['Quantity', 'Assignment', 'On', 'From'],
        });
        let output = {};

        _.each(result.spies, (spy) => {
          let key = [spy.assignment, spy.assigned_to.name, spy.based_from.name].join(',');
