...
sub www_send_test_message {
    ...
    my $empire = Lacuna->db->resultset('Empire')->find($id);
...
sub www_search_essentia_codes {
    ...
    my $codes = Lacuna->db->resultset('EssentiaCode')->search(undef, {order_by => { -desc => 'date_created' }, rows => 25, page => $page_number });
...
sub www_view_essentia_log {
    ...
    my $transactions = Lacuna->db->resultset('Log::Essentia')->search({empire_id => $empire_id}, {order_by => { -desc => 'date_stamp' }});
...
sub www_view_login_log {
    ...
    my $page_number = $request->param('page_number') || 1;
    my $logins = Lacuna->db->resultset('Log::Login')->search(
...
sub www_view_empire_name_change_log {
    my ($self, $request) = @_;
    my $empire_id = $request->param('empire_id');
    my $history = Lacuna->db->resultset('Log::EmpireNameChange')->search({empire_id => $empire_id},{order_by => { -desc => 'date_stamp' }});
...
sub www_search_similar_empire {
    ...
    my $search = Lacuna->db->resultset('Empire')->search(
        { -and => \@query },
        { order_by => { -desc => 'id' },
          rows     => 25,
          page     => $page_number,
...
sub www_search_empires {
    my ($self, $request) = @_;
    my $page_number = $request->param('page_number') || 1;
    my $empires = Lacuna->db->resultset('Empire')->search(undef, { rows => 25, page => $page_number });
...
sub www_search_bodies {
    my ($self, $request) = @_;
    my $page_number = $request->param('page_number') || 1;
    my $bodies = Lacuna->db->resultset('Map::Body')->search(undef, {order_by => ['me.name'], rows => 25, page => $page_number, prefetch=>[qw/empire star/] });
    my $name = decode_utf8($request->param('name') || '');
    my $pager = 'name';
...
sub www_search_stars {
    my ($self, $request) = @_;
    my $page_number = $request->param('page_number') || 1;
    my $stars = Lacuna->db->resultset('Map::Star')->search(undef, {order_by => ['name'], rows => 25, page => $page_number });
    my $name = decode_utf8($request->param('name') || '');
    if ($name) {
        my $query = "$name%";
        $query =~ s/\*/%/;
...
sub www_view_buildings {
    my ($self, $request, $body_id) = @_;
    $body_id ||= $request->param('body_id');
    my $buildings = Lacuna->db->resultset('Building')->search({ body_id => $body_id }, {order_by => ['x','y'] });
...
sub www_set_efficiency {
    my ($self, $request) = @_;
    my $building = Lacuna->db->resultset('Building')->find($request->param('building_id'));
...
sub www_view_fleets {
    ...
    while (my $fleet = $fleets->next) {
        ...
        if ($fleet->task eq 'Travelling') {
            ...
            my $target = Lacuna->db->resultset('Map::Body')->find($fleet->foreign_body_id);
...
sub www_view_glyphs {
    my ($self, $request, $body_id) = @_;
    $body_id ||= $request->param('body_id');
    my $glyphs = Lacuna->db->resultset('Glyph')->search({ body_id => $body_id }, {order_by => ['type'] });
...
<input type="submit" value="add essentia"></form></td></tr>', $empire->id);
    ...
    my $invites_sent = Lacuna->db->resultset('Invite')->search({inviter_id => $empire->id});
    $out .= join ' ; ',
        map {
    ...
    my $invite_accepted = Lacuna->db->resultset('Invite')->search({invitee_id => $empire->id})->first;
...
sub www_set_alliance_logo {
    my ($self, $request, $id) = @_;
    $id ||= $request->param('id');
    my $alliance = Lacuna->db->resultset('Alliance')->find($id);
    ...
    my $full_url = Lacuna->config->get('assets_url').'alliances/'.$image.'.png';
...
sub www_view_body {
    my ($self, $request, $id) = @_;
    $id ||= $request->param('id');
    my $body = Lacuna->db->resultset('Map::Body')->find($id);
    unless (defined $body) {
        confess [404, 'Body not found.'];
    }
...
sub www_view_star {
    my ($self, $request, $id) = @_;
    $id ||= $request->param('id');
    my $star = Lacuna->db->resultset('Map::Star')->find($id);
    unless (defined $star) {
...
sub www_view_virality {
    ...
    my $past30 = Lacuna->db->resultset('Log::Viral')->search({date_stamp => { '>=' => $dt_formatter->format_datetime(DateTime->now->subtract(days => 31))}}, { order_by => 'date_stamp'});
...
sub www_view_economy {
    ...
    my $past30 = Lacuna->db->resultset('Log::Economy')->search({date_stamp => { '>=' => $dt_formatter->format_datetime(DateTime->now->subtract(days => 31))}}, { order_by => 'date_stamp'});
    while (my $day = $past30->next) {
        unless (defined $previous) {
            $previous = $day;
            next;
...
sub www_server_wide_recalc {
    my ($self, $request) = @_;
    Lacuna->db->resultset('Map::Body')->search({empire_id => {'>', 0}})->update({needs_recalc=>1});
...
sub www_delambert {
    ...
    my ($scratch) = Lacuna->db->resultset('AIScratchPad')->search({ai_empire_id => -9, body_id => 0});
...
sub www_delambert_war {
    ...
    my @ai_defence = Lacuna->db->resultset('AIBattleSummary')->search({
        defending_empire_id => -9,
    });
    my @ai_attack = Lacuna->db->resultset('AIBattleSummary')->search({
...
ATTACKER:
    foreach my $attacker (@worst_attackers) {
        my $attack_empire = Lacuna->db->resultset('Empire')->find($attacker);
        next ATTACKER unless $attack_empire;

        # Obtain all colonies of the attacking empire, sorted by population desc.
        my @colonies = Lacuna->db->resultset('Map::Body')->search({
            empire_id       => $attacker,
        });
...
sub wrap {
    ...
    my $server_url = URI->new(Lacuna->config->get('server_url'));
