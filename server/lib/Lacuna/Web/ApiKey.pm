...
sub www_view_stats {
    my ($self, $request) = @_;
    my $pair = Lacuna->db->resultset('ApiKey')->search({ private_key => $request->param('private_key')})->first;
    unless (defined $pair) {
    ...
    my $login = Lacuna->db->resultset('Log::Login')->search({api_key => $pair->public_key});
    ...
    my $essentia = Lacuna->db->resultset('Log::Essentia')->search({api_key => $pair->public_key, amount => { '<' => 0 } });
    $out .= $row->('Essentia Spent',
        $essentia->search({date_stamp => { '>=' =>$thirty_days_ago }})->get_column('amount')->sum * -1,
        $essentia->get_column('amount')->sum * -1
    );
    my $lottery = Lacuna->db->resultset('Log::Lottery')->search({api_key => $pair->public_key });
    $out .= $row->('Lottery Votes',
        $lottery->search({date_stamp => { '>=' =>$thirty_days_ago }})->count,
        $lottery->count
    );
    my $rpc = Lacuna->db->resultset('Log::RPC')->search({api_key => $pair->public_key });
    $out .= $row->('RPC Calls',
...
sub www_generate_key {
    my ($self, $request) = @_;
    my $pair = Lacuna->db->resultset('ApiKey')->new({
