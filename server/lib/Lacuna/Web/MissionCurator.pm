...
sub www_add_essentia {
    ...
    my $recent = Lacuna->db->resultset('Log::Essentia')->search({
...
sub www_payouts {
    my ($self, $request) = @_;
    my $payouts = Lacuna->db->resultset('Log::Essentia')->search(
...
sub www_default {
    ...
    my $empires = Lacuna->db->resultset('Empire')->search(undef, {order_by => ['name'], rows => 25, page => $page_number });
