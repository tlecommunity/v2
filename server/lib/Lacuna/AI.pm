...
has scratch     => (
    ...
    default     => sub {
        my $self = shift;
        my ($scratch) = Lacuna->db->resultset('AIScratchPad')->search({
...
sub build_colony {
    ...
    my $plot_use = 0;
    my $buildings = Lacuna->db->resultset('Building');
...
sub run_missions {
    ...
    my $infiltrated_spies = Lacuna->db->resultset('Spy')->search({from_body_id => $colony->id, on_body_id => {'!=', $colony->id}});
...
sub train_spies {
    ...
    if ($subsidise) {
        ...
        while ($train_count < $room_for) {
            ...
            my $spy = Lacuna->db->resultset('Spy')->new({
                from_body_id    => $colony->id,
...
sub build_ships {
    ...
    my $ships = Lacuna->db->resultset('Fleet');
...
sub set_defenders {
    my ($self, $colony) = @_;
    say 'SET DEFENDERS';
    my $local_spies = Lacuna->db->resultset('Spy')->search({empire_id => $colony->empire_id, on_body_id => $colony->id});
    my $on_sweep = Lacuna->db->resultset('Spy')->search({empire_id => $colony->empire_id, on_body_id => $colony->id, task => "Security Sweep"})->count;
    my $enemies = Lacuna->db->resultset('Spy')->search({on_body_id => $colony->id, task => { '!=' => 'Captured'}, empire_id => { '!=' => $self->empire_id }})->count;
        ...
        if ($spy->is_available) {
            if ( $spy->date_created < DateTime->now->subtract(hours => 8) and
                ...
                if ($spy_result->{message_id}) {
                    my $message = Lacuna->db->resultset('Message')->find($spy_result->{message_id});
...
#When is in hours from prisoner being released.
    ...
    say 'KILL PRISONERS';
    my $prisoners = Lacuna->db->resultset('Spy')->search({on_body_id => $colony->id, task => 'Captured', empire_id => { '!=' => $self->empire_id }});
    my $now = DateTime->now;
    my $prisoner_cnt = 0;
