...
__PACKAGE__->has_many('spies',              'Lacuna::DB::Result::Spy',          'empire_id');
...
__PACKAGE__->has_many('taxes',              'Lacuna::DB::Result::Tax',          'empire_id');
...
__PACKAGE__->has_many('medals',             'Lacuna::DB::Result::Medal',        'empire_id');
__PACKAGE__->has_many('all_probes',         'Lacuna::DB::Result::Probe',        'empire_id');
...
sub _build_db {
    ...
    return Lacuna::SDB->instance->db;
}


# I'm the sitter for these babies.
__PACKAGE__->has_many('allbabyauths', 'Lacuna::DB::Result::SitterAuth', 'sitter_id');
__PACKAGE__->has_many('babyauths',    'Lacuna::DB::Result::SitterAuth', sub {
                            my $args = shift;
                            return (
                                    {
...
__PACKAGE__->has_many('allsitterauths', 'Lacuna::DB::Result::SitterAuth', 'baby_id');
__PACKAGE__->has_many('sitterauths',    'Lacuna::DB::Result::SitterAuth', sub {
                            my $args = shift;
                            return (
                                    {
...
sub _build_is_active {
    my ($self) = @_;
    Lacuna::Cache->instance->get('empire_active', $self->id);
}

sub set_active {
    my ($self) = @_;
    Lacuna::Cache->instance->set('empire_active', $self->id, 1, 30 * 60);
...
sub determine_species_limits {
    ...
    my $colonies = $self->db->resultset('Map::Body')->search({ empire_id => $self->id, class => { '!=' => 'Lacuna::DB::Result::Map::Body::Planet::Station'} });
    ...
    elsif (Lacuna::Cache->instance->get('redefine_species_timeout', $self->id)) {
...
sub add_medal {
    ...
    if ($send_message && !$self->skip_medal_messages) {
        my $name = $medal->name;
        my $image = Lacuna->config->get('assets_url').'medal/'.$type.'.png';
...
sub rpc_count {
    my $self = shift;
    Lacuna::Cache->instance->get('rpc_count_'.format_date(undef,'%d'), $self->id);
}

sub inc_rpc_count {
    my $self = shift;
    return Lacuna::Cache->instance->incr('rpc_count_'.format_date(undef,'%d'), $self->id, 1, 60 * 60 * 30);
}
...
sub reset_rpc {
    my ($self) = @_;
    my $cache  = Lacuna::Cache->instance;
    my $id     = $self->id;
...
has rpc_limit => (
    ...
    default => sub {
        my ($self) = @_;
        return Lacuna::Cache->instance->get('rpc_count_'.format_date(undef,'%d'), $self->id, 1, 60 * 60 * 30);
    }
);
...
has rpc_rate => (
    ...
    default => sub {
        my $self = shift;
        return Lacuna::Cache->instance->incr('rpc_rate_'.format_date(undef,'%M'), $self->id, 1, 60 * 2);
    }
);
...
sub attach_invite_code {
    ...
    if (defined $invite_code && $invite_code ne '') {
        ...
        if (defined $invite) {
            ...
            Lacuna::Cache->instance->incr('friends_accepted', format_date(undef,'%F'), 1, 60 * 60 * 26);
...
sub invite_friend {
    ...
    Lacuna::Cache->instance->incr('friends_invited', format_date(undef,'%F'), 1, 60 * 60 * 26);
...
sub send_message {
    ...
    if (exists $params{repeat_check}) {
        Lacuna::Cache->instance->set($params{repeat_check}, int(DateTime->now->hour / 6), 1, 60 * 60 * 6);
    }
...
sub check_for_repeat_message {
    my ($self, $repeat) = @_;
    return Lacuna::Cache->instance->get($repeat, int(DateTime->now->hour / 6));
}
