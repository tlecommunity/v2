...
sub get_spies {
    my ($self) = @_;
    return Lacuna->db->resultset('Spy')->search({ from_body_id => $self->body_id });
}

sub get_empire_spies {
    my ($self) = @_;
    return Lacuna->db->resultset('Spy')->search({ empire_id => $self->body->empire_id });
}

sub get_spy {
    my ($self, $spy_id) = @_;
    my $spy = Lacuna->db->resultset('Spy')->find($spy_id);
    unless (defined $spy) {
