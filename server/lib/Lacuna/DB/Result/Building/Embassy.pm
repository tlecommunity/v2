...
sub create_alliance {
    ...
    my $alliances = Lacuna->db->resultset('Alliance');
...
sub get_my_invites {
    my $self = shift;
    my $invites = Lacuna->db->resultset('AllianceInvite')->search({empire_id => $self->body->empire_id});
    my @out;
...
sub exchange_with_stash {
    ...
    Lacuna->cache->incr('stash_exchanges_'.format_date(undef,'%d'), $self->body_id, 1, 60 * 60 * 26);
...
sub propositions {
    my ($self) = @_;
    my $alliance = $self->alliance;
    return Lacuna->db->resultset('Proposition')->
