...
sub platforms {
    my $self = shift;
    return Lacuna->db->resultset('MiningPlatform')->search({ planet_id => $self->body_id });
}

sub ships {
    my $self = shift;
    return Lacuna->db->resultset('Fleet')->search({ body_id => $self->body_id, task => 'Mining' });
}
...
sub can_add_platform {
    ...
    unless ($on_arrival) {
        $count += Lacuna->db->resultset('Fleet')->search({type=>'mining_platform_ship', task=>'Travelling',body_id=>$self->body_id})->count;
    }
    ...
    # asteroid count
    $count = Lacuna->db->resultset('MiningPlatform')->search({ asteroid_id => $asteroid->id })->count;
    unless ($on_arrival) {
        $count += Lacuna->db->resultset('Fleet')->search({type=>'mining_platform_ship',foreign_body_id => $asteroid->id, task=>'Travelling',body_id=>$self->body_id})->count;
    }
...
sub add_platform {
    my ($self, $asteroid) = @_;
    Lacuna->db->resultset('MiningPlatform')->new({
