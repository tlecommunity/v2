...
# Check to see if excavs available
  my $avail = Lacuna->db->resultset('Fleet')
...
sub excavators {
  my $self = shift;
  return Lacuna->db->resultset('Excavator')->search({ planet_id => $self->body_id });
}
...
sub can_add_excavator {
  ...
  my $travel;
  unless ($on_arrival) {
    $travel = Lacuna->db->resultset('Fleet')
...
# Allowed one per empire per body.
  $count = Lacuna->db->resultset('Excavator')
...
sub add_excavator {
  my ($self, $body) = @_;
  Lacuna->db->resultset('Excavator')->new({
    ...
    if ($self->is_glyph_found) {
        ...
        $empire->send_predefined_message(
            ...
            attachments => {
                image => {
                    title   => $ore,
                    url     => Lacuna->config->get('assets_url').'glyphs/'.$ore.'.png',
                }
            }
        );
