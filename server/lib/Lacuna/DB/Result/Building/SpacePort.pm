...
sub battle_logs {
    my ($self) = @_;
    return Lacuna->db->resultset('Log::Battles')->search(
        {
            -or => [
...
has max_ships => (
    ...
    default => sub {
        my $self = shift;
        my $levels = Lacuna->db->resultset('Building')->search( {
            class       => $self->class,
            body_id     => $self->body_id,
            efficiency  => 100,
...
sub find_ship {
    my ($self, $type) = @_;
    my $ship = Lacuna->db->resultset('Fleet')->search({body_id => $self->body_id, task => 'Docked', type => $type} )->first;
    unless (defined $ship ) {
        $type =~ s/_/ /g;
...
before delete => sub {
  my ($self) = @_;
  unless (Lacuna->db->resultset('Building')
