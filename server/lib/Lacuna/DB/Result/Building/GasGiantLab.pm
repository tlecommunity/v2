...
before has_special_resources => sub {
    ...
    if ($planet->get_stored('rutile') + $planet->get_stored('chromite') + $planet->get_stored('bauxite') + $planet->get_stored('magnetite') + $planet->get_stored('beryl') + $planet->get_stored('goethite') < $amount_needed) {
