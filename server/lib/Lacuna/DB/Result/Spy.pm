    ...
    if ($task ~~ ['Idle',
        ...
        if ($train_bld) {
            ...
            if ($self->$tr_skill >= $max_points) {
                ...
                my $train_cnt  = $db->resultset('Spy')->search({task => "$task",
...
#Clean escape
    ...
    my $spy_pod = Lacuna->db->resultset('Fleet')->new({
                    body_id         => $self->from_body_id,
                    type            => 'spy_pod',
                    name            => "Bugout",
                })->insert;
...
sub steal_planet {
    ...
    my $defender_capitol_id = $self->on_body->empire->home_planet_id;
    Lacuna->db->resultset('Spy')->search({
        on_body_id => $self->on_body_id,
        task => { 'in' => [ 'Training',
                            'Intel Training',
                            'Mayhem Training',
    ...
    my $spies = Lacuna->db->resultset('Spy')
...
sub travel_report {
    ...
    $ships = Lacuna->db->resultset('Fleet')->search(
        {body_id => $self->on_body->id,
         task => {'in' => ['Supply Chain', 'Waste Chain'] }},
        );
    while (my $ship = $ships->next) {
