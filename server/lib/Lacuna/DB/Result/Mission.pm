...
has params => (
    ...
    default => sub {
        my $self = shift;
        return Config::JSON->new('/home/lacuna/mission/missions/'. $self->mission_file_name);
    },
);

sub log {
    my $self = shift;
    my $logs = Lacuna->db->resultset('Log::Mission');
...
sub check_objectives {
    ...
    if (exists $objectives->{glyphs}) {
        ...
        foreach my $glyph (@{$objectives->{glyphs}}) {
            ...
            my $glyph_on_body = Lacuna->db->resultset('Glyph')->search({
                type    => $glyph->{type},
                body_id => $body->id,
            })->first;
    ...
    if (exists $objectives->{fleet_movement}) {
        ...
        my $stars = Lacuna->db->resultset("Map::Star");
...
sub initialize {
    ...
    return undef unless (-f '/home/lacuna/mission/missions/'.$filename);
    my $mission = Lacuna->db->resultset('Mission')->new({
        zone                => $zone,
...
sub find_body_target {
    ...
    my $body = Lacuna->db->resultset('Map::Body')->search({size => { between => $movement->{target}{size}}});
...
sub find_star_target {
    my ($class, $movement, $zone) = @_;
    my $star = Lacuna->db->resultset('Map::Star');
