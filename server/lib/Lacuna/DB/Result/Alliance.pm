...
sub add_member {
    ...
    $empire->update;
    # I has your probes!
    Lacuna->db->resultset('Probe')->search({empire_id => $empire->id})->update({alliance_id => $self->id});
...
#Send email to all alliance members notifying them of change in leadership.
    ...
    $empire->update;
    Lacuna->db->resultset('Probe')->search({empire_id => $empire->id})->update({alliance_id => undef});
    return $self;
}
