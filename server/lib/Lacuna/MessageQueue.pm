...
use Lacuna::Queue;
use Lacuna::MessageQueue::Context;
...
sub log {
    my ($self) = @_;
    return Log::Log4perl->get_logger( "Lacuna::MessageQueue::".$self->name );
}
...
sub route_call {
    ...
    eval {
        ...
        $self->log->debug("route = [$route]");
        my $context = Lacuna::MessageQueue::Context->new({
            name        => $self->name,
            content     => $content,
            msg_id      => $msg_id,
            user_id     => $user_id,
