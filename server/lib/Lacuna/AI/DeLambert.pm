...
sub check_enemy_spy_action {
    ...
    my $enemy_spies = Lacuna->db->resultset('Spy')->search({
    ...
    for my $empire_id (keys %$spy_ref) {
        my ($ai_battle_summary) = Lacuna->db->resultset('AIBattleSummary')->search({
            attacking_empire_id => $empire_id,
            defending_empire_id => -9,
        });
        if (not $ai_battle_summary) {
            $ai_battle_summary = Lacuna->db->resultset('AIBattleSummary')->create({
                attacking_empire_id => $empire_id,
                defending_empire_id => -9,
                attack_victories    => 0,
                defense_victories   => 0,
...
#          glyph_id => 0,
        ...
        if ($quantity) {
            ...
            Lacuna->db->resultset('Market')->create(\%trade);
        }
    }
}
...
sub get_trade_ship {
    ...
    my ($ship) = Lacuna->db->resultset('Fleet')->search({
        task    => 'Docked',
        type    => 'galleon',
        body_id => $colony->id,
    });
...
sub buy_trade {
    ...
    my $market = Lacuna->db->resultset('Market')->search({
    ...
    if ($best_price_per_plan <= $scratchpad->{buy_max_price_per_plan}) {
        my $trade = Lacuna->db->resultset('Market')->find($best_trade_id);
...
sub retaliate {
    ...
    my @del_colonies = Lacuna->db->resultset('Map::Body')->search({
        empire_id   => -9,
    });
...
TARGET:
    foreach my $target_id (keys %$attack) {
       my $target = Lacuna->db->resultset('Empire')->find($target_id);
       if ($target) {
           ...
           if ($freq eq 'hourly' or $freq eq 'once' or ($freq eq 'daily' and $attack_daily)) {
               ...
               my $target_colony       = Lacuna->db->resultset('Map::Body')->find($target_colony_id);
...
#        tag         => 'Correspondence',
    ...
    while (my $message = $messages->next) {
        ...
        my $request_empire = Lacuna::db->resultset('Empire')->find($message->from_id);
