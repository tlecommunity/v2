...
#   not an asteroid or uninhabited planet
    ...
    if ($is_asteroid) {
        my $platforms = Lacuna->db->resultset('MiningPlatform')->search({asteroid_id => $self->foreign_body_id });
    ...
    # Destroy Excavs
    my $excavs = Lacuna->db->resultset('Excavator')->search({body_id => $self->foreign_body_id });
