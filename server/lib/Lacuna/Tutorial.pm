...
sub counter_spy {
    ...
    my $home = $empire->home_planet;
    if ($finish) {
        my $counter = Lacuna->db->resultset('Spy')->search({empire_id => $empire->id, task=>'Counter Espionage'})->count;
        if ($counter >= 2) {
            $home->add_food_type('trona', 700);
            $home->add_food_type('bread', 700);
            $home->add_energy(700);
...
sub observatory {
    ...
    if ($finish) {
        ...
        if (defined $building && $building->level >= 1) {
            ...
            if (defined $shipyard) {
                $shipyard->body($home);
                my $probe = Lacuna->db->resultset('Fleet')->new({body=>$home, type=>'probe', quantity => 1,});
...
sub the_end {
    ...
    if ($finish) {
        ...
        my $ssd = Lacuna->db->resultset('Building')->new({
           x        => $x,
           y        => $y,
