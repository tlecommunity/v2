package Lacuna::App::MQWorker::Daemon;
...
use Lacuna::MessageQueue;

extends 'Lacuna::App::MQWorker';
...
sub run {
    ...
    my $log = Log::Log4perl->get_logger('Lacuna::MessageQueue');

    print "Run the Message Queue daemon [".$self->nodaemonize."]\n";

#    $App::Daemon::loglevel = $self->verbose ? $DEBUG : $WARN;
#    $App::Daemon::logfile = '/home/lacuna/server/log/MQWorkerDaemon.log';

    my $pid_file = '/home/lacuna/server/log/MQWorkerDaemon.pid';
    my $start = time;
    ...
    my $message_queue   = Lacuna::MessageQueue->new({
        name    => 'bg_worker',
    });
