...
sub get_mining_platforms_for_asteroid_in_jurisdiction {
    ...
    my $platforms = Lacuna->db->resultset('MiningPlatform')->search({asteroid_id => $asteroid->id});
...
sub propose_evict_mining_platform {
    ...
    my $platform = Lacuna->db->resultset('MiningPlatform')->find($platform_id);
...
sub propose_evict_excavator {
    ...
    my $excav = Lacuna->db->resultset('Excavator')->find($excav_id);
...
sub view_taxes_collected {
    ...
    my $building = $session->current_building;
    my @out;
    my $taxes = Lacuna->db->resultset('Tax')->search({station_id => $building->body_id});
