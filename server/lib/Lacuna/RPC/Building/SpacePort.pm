...
sub prepare_fetch_spies {
    ...
    my $spies_rs = Lacuna->db->resultset('Spy')->search({
        on_body_id  => $on_body->id,
        empire_id   => $empire->id,
        -or => [
...
sub _fleet_filter_options {
    ...
    for my $type ( SHIP_TYPES ) {
        my $fleet = Lacuna->db->resultset('Fleet')->new({ type => $type });
...
sub view_mining_platforms {
    ...
    my $target      = $self->find_target($args{target});
    my $platform_rs = Lacuna->db->resultset('MiningPlatform');
...
sub view_excavators {
    ...
    my $target      = $self->find_target($args{target});
    my $excavator_rs = Lacuna->db->resultset('Excavator');
