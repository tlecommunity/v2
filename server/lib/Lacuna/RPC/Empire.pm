...
sub login {
    ...
    if ($empire->rpc_rate > $throttle) {
        Lacuna->cache->incr('rpc_limit_'.format_date(undef,'%d'), $empire->id, 1, 60 * 60 * 30);
...
sub create {
    ...
    Lacuna->cache->incr('empires_created', format_date(undef,'%F'), 1, 60 * 60 * 26);
    ...
    #
    Lacuna->db->resultset('User')->create({
        id          => $empire->id,
...
sub get_public_profile {
    ...
    my @colonies;
    my $probes = Lacuna->db->resultset('Probe');
...
sub authorize_sitters {
    ...
    my $rs = $baby->sitters;
    my $auths = Lacuna->db->resultset('SitterAuth');
...
sub deauthorize_sitters {
    ...
    my $rs = Lacuna->db->resultset('SitterAuth');
