...
sub send_message {
    ...
    if ($options->{in_reply_to}) {
        my $reply_to = Lacuna->db->resultset('Message')->find($options->{in_reply_to});
    ...
    my $attachments = {};
    if ($options->{forward}) {
        my $forward = Lacuna->db->resultset('Message')->find($options->{forward});
    ...
    foreach my $name (split /\s*,\s*/, $recipients) {
        ...
        if ($name eq '@ally') {
            ...
            my $user = Lacuna->db->resultset('Empire')->search({name => $name})->first;
            if (defined $user) {
                push @sent, $user->name;
                push @to, $user;
                $send_count++;
