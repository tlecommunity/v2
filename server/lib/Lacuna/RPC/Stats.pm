...
sub alliance_rank {
    ...
    my $ranks = Lacuna->db->resultset('Log::Alliance');
    unless ($page_number) {
...
sub find_alliance_rank {
    ...
    my $session  = $self->get_session({session_id => $session_id });
    my $empire   = $session->current_empire;
    my $ranks = Lacuna->db->resultset('Log::Alliance')->search(undef,{order_by => $by, rows=>25});
...
sub find_empire_rank {
    ...
    my $session  = $self->get_session({session_id => $session_id });
    my $empire   = $session->current_empire;
    my $ranks = Lacuna->db->resultset('Log::Empire')->search(undef,{order_by => $by, rows=>25});
...
sub colony_rank {
    ...
    my $ranks = Lacuna->db->resultset('Log::Colony')->search(undef,{order_by =>$by, rows=>25});
    my @colonies;
    while (my $rank = $ranks->next) {
        push @colonies, {
            empire_id                   => $rank->empire_id,
...
sub spy_rank {
    ...
    my $ranks = Lacuna->db->resultset('Log::Spy')->search(undef,{order_by => $by, rows=>25});
    my @spies;
    while (my $rank = $ranks->next) {
        push @spies, {
            empire_id                   => $rank->empire_id,
...
sub weekly_medal_winners {
    ...
    my $session  = $self->get_session({session_id => $session_id });
    my $empire   = $session->current_empire;
    my $winner_rs = Lacuna->db->resultset('Log::WeeklyMedalWinner')->search;
    my @winners;
