package UnitTestsFor::Lacuna::WebSocket::User;
...
use Lacuna::ClientCode;
use Lacuna::EmailCode;
...
sub test_construction {
    ...
    my $ws_user = Lacuna::WebSocket::User->new;
    isa_ok($ws_user, 'Lacuna::WebSocket::User');
}

sub test_ws_clientCode {
    my ($self) = @_;

    my $context = Lacuna::WebSocket::Context->new({
        client_code     => 'invalid',
        msg_id          => 123,
    });
    my $ws_user = Lacuna::WebSocket::User->new;
...
sub test_ws_forgotPassword {
    ...
    is($@->[0], 1001, "Code");
    like($@->[1], qr/^Client Code is invalid/, "Message");
    $context->client_code(Lacuna::ClientCode->new->id);
...
sub test_ws_loginWithEmailCode {
    ...
    my $email_code = Lacuna::EmailCode->new({ user_id => 1 });
    ...
    $fixtures->load('user_bernard');
    $email_code = Lacuna::EmailCode->new({ user_id => 2 });
    $content->{emailCode} = $email_code->id;
