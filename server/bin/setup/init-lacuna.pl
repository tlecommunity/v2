...
# /home/lacuna/server/etc/reboot.conf's db->dsn field to a new db first.
...
sub update_database_chunk {
    ...
    my $create_lacunans = 1 if ($p == 0 and $q == 0);
    ...
    foreach my $star_xy (@stars_xy) {
        ...
        add_star_system({
            ...
            create_lacunans     => $create_lacunans,
        });
    }
}
...
sub add_star_system {
    ...
    my $create_lacunans     = $args->{create_lacunans};
    ...
    for my $orbit (1..8) {
        ...
        if (randint(1,100) <= 10) {
            ...
            if ($body_name =~ m/^P/ and $create_lacunans) {
