...
my $app = builder {
    if ($^O ne 'darwin' && not defined $ENV{'LACUNA_NO_MIDDLEWARE'} ) {
    ...
    mount '/admin' => builder {
        enable "Auth::Basic", authenticator => sub {
            ...
            my $empire = Lacuna->db->resultset('Empire')->search({name => $username, is_admin => 1})->first;
    ...
    mount "/missioncurator" => builder {
        enable "Auth::Basic", authenticator => sub {
            my ($username, $password) = @_;
            return 0 unless $username;
            my $empire = Lacuna->db->resultset('Empire')->search({name => $username, is_mission_curator => 1})->first;
            return 0 unless defined $empire;
            return $empire->is_password_valid($password);
        };
